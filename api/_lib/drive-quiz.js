...
const HTTP_MS=9000,MAX_FILE_BYTES=5_000_000,MAX_SOURCE_TEXT=18000,MAX_FILES_PER_RUN=5;
...
export async function listQuizDocuments(limit=MAX_FILES_PER_RUN){
  const key=apiKey(),folder=quizFolderId();if(!key)return{configured:false,folder,files:[],reason:'missing_google_drive_api_key'};
  const url=new URL('https://www.googleapis.com/drive/v3/files');
  url.searchParams.set('q',`'${folder}' in parents and trashed=false`);url.searchParams.set('fields','files(id,name,mimeType,modifiedTime,size,webViewLink)');url.searchParams.set('pageSize',String(Math.max(1,Math.min(20,limit))));url.searchParams.set('orderBy','modifiedTime desc');url.searchParams.set('key',key);
  const response=await withTimeout(url,{headers:{accept:'application/json','user-agent':'HIU-YHCT-Quiz-Ingest/1.0'}});if(!response.ok)throw new Error(`Google Drive list ${response.status}`);
  const payload=await response.json();const files=(Array.isArray(payload?.files)?payload.files:[]).filter(f=>String(f.mimeType||'')==='application/vnd.openxmlformats-officedocument.wordprocessingml.document'||String(f.name||'').toLowerCase().endsWith('.docx'));
  return{configured:true,folder,files:files.slice(0,Math.max(1,Math.min(MAX_FILES_PER_RUN,limit)))};
}
...
const startQuestion=line=>{
  const m=line.match(/^\s*(?:câu\s*)?(\d{1,4})\s*[\.)\]:-]\s*(.+)$/i);return m?{n:m[1],stem:clean(m[2],4000)}:null;
};
