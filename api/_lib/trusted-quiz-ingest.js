...
const MAX_UPLOAD_BYTES=2_000_000,MAX_DRIVE_BYTES=5_000_000,MAX_SOURCE_TEXT=400_000,HTTP_MS=9000;
...
const isPdf=file=>String(file?.mimeType||'')===PDF_MIME||/\.pdf$/i.test(String(file?.name||''));
const isText=file=>String(file?.mimeType||'')===TEXT_MIME||/\.txt$/i.test(String(file?.name||''));
const isGoogleDoc=file=>String(file?.mimeType||'')===GDOC_MIME;
const isSupportedSource=file=>isDocx(file)||isPdf(file)||isText(file)||isGoogleDoc(file);
const mimeForFile=file=>isDocx(file)?DOCX_MIME:isPdf(file)?PDF_MIME:isText(file)?TEXT_MIME:isGoogleDoc(file)?GDOC_MIME:clean(file?.mimeType,180)||'application/octet-stream';
const subjectFromName=name=>clean(String(name||'').replace(/\.(?:docx|pdf|txt)$/i,'').replace(/^\s*\d+[._ -]*/,'').replace(/[_-]+/g,' '),160)||'Chưa phân loại';
...
async function listQuizBankCandidates(){
  ...
  for(const file of root.filter(isSupportedSource))addDirect(file);
  ...
  for(const child of rootFolders.filter(x=>normalizedName(x.name)!==normalizedName(MANUAL_INTAKE_FOLDER)&&!isTechnicalFolder(x.name))){
    const nested=await listChildren(child.id,1000).catch(()=>[]),docs=nested.filter(isSupportedSource);if(docs.length)subjects.add(clean(child.name,160));for(const file of docs)files.push({...file,parentName:clean(child.name,160)});
  }

  let manualDirect=0;
  if(manual){
    const rows=await listChildren(manual.id,1000),direct=rows.filter(isSupportedSource);manualDirect=direct.length;
    ...
    for(const child of subjectFolders){const nested=await listChildren(child.id,1000).catch(()=>[]),docs=nested.filter(isSupportedSource);if(docs.length)subjects.add(clean(child.name,160));for(const file of docs)files.push({...file,parentName:clean(child.name,160)})}
  }
  ...
  return{configured:true,folder,folderName:BANK_FOLDER_NAME,subjects:subjectList,ungroupedFiles:root.filter(isSupportedSource).length+manualDirect,files:dedup.slice(0,1000)};
}

async function downloadSource(file){
  const isGdoc=isGoogleDoc(file),url=new URL(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(file.id)}${isGdoc?'/export':''}`);if(isGdoc)url.searchParams.set('mimeType',TEXT_MIME);else url.searchParams.set('alt','media');
  const response=await driveFetch(url,{},12000);if(!response.ok)throw new Error(`Google Drive download ${response.status}`);const len=Number(response.headers.get('content-length')||file.size||0);if(len>MAX_DRIVE_BYTES)throw new Error('Tài liệu vượt giới hạn 5 MB');const buffer=Buffer.from(await response.arrayBuffer());if(!buffer.length||buffer.length>MAX_DRIVE_BYTES)throw new Error('Tài liệu rỗng hoặc vượt giới hạn 5 MB');return buffer;
}

async function sourceText(buffer,file){
  let text='';
  if(isDocx(file))text=inspectMarkedDocx(buffer).map(x=>x.text).join('\n');
  else if(isText(file)||isGoogleDoc(file))text=buffer.toString('utf8');
  else if(isPdf(file)){
    const {PDFParse}=await import('pdf-parse'),parser=new PDFParse({data:buffer});
    try{const result=await parser.getText();text=String(result?.text||'')}finally{await parser.destroy().catch(()=>{})}
  }else throw new Error('Chỉ hỗ trợ DOCX, PDF, TXT hoặc Google Docs.');
  text=String(text||'').replace(/\r\n?/g,'\n').replace(/\u0000/g,'').trim();if(text.length>MAX_SOURCE_TEXT)throw new Error('Tài liệu quá dài; hãy chia nhỏ trước khi cập nhật.');return text;
}

async function parseTrustedDeterministicSource(buffer,file,sourceHash){
  const plain=await sourceText(buffer,file),explicit=parseMcqDocument(plain),marked=isDocx(file)?parseTrustedMarkedDocx(buffer,file,sourceHash):{questions:[],total:0,invalid:[],marker:'explicit-answer-key-v1'},byHash=new Map();
...
async function importTrusted(req,file,buffer){
  const sourceHash=sha256(buffer),subjectHint=subjectForFile(file),sourceFile={...file,parentName:subjectHint},parsed=await parseTrustedDeterministicSource(buffer,sourceFile,sourceHash);
  ...
  const meta=documentMeta(file,sourceHash,parsed.questions.length,subjectHint,parsed.invalid.length?'needs_review':'ready',parsed.invalid.length?`Trusted partial source: ${parsed.valid} câu hợp lệ, ${parsed.invalid.length} câu chưa có đáp án xác định.`:'');
...
function uploadedSource(body){
  const name=clean(body.fileName,300),base64=String(body.base64||''),extension=(name.match(/\.(docx|pdf|txt)$/i)?.[1]||'').toLowerCase();if(!extension)throw new Error('Tài liệu phải là DOCX, PDF hoặc TXT');if(!base64||base64.length>2_800_000||!/^[A-Za-z0-9+/]*={0,2}$/.test(base64))throw new Error('Tệp không hợp lệ hoặc vượt 2 MB');const buffer=Buffer.from(base64,'base64');if(!buffer.length||buffer.length>MAX_UPLOAD_BYTES)throw new Error('Tệp phải nhỏ hơn 2 MB');const sourceHash=sha256(buffer),subject=clean(body.subjectName,160)||subjectFromName(name),mimeType=extension==='docx'?DOCX_MIME:extension==='pdf'?PDF_MIME:TEXT_MIME;return{file:{id:`upload-trusted-${sourceHash}`,name,mimeType,parentName:subject,createdTime:new Date().toISOString(),modifiedTime:new Date().toISOString()},buffer};
}
...
export async function handleTrustedQuizIngest(req,res){
  ...
  try{
    if(action==='trusted-quiz-upload'){
      const {file,buffer}=uploadedSource(req.body||{}),result=await importTrusted(req,file,buffer);return res.status(result.ok?200:422).json(result);
    }
    if(action==='trusted-quiz-sync'){
      const listing=await listQuizBankCandidates();
      ...
      for(const file of selected){try{processed.push(await importTrusted(req,file,await downloadSource(file)))}catch(error){processed.push({ok:false,status:'error',fileName:file.name,subject:subjectForFile(file),message:clean(error?.message||'Không xử lý được tài liệu',300)})}}
