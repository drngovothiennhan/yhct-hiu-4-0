...
const PARSER_REVISION='red-layout-v4';
...
const isGoogleDoc=file=>String(file?.mimeType||'')===GOOGLE_DOC_MIME;
const isConvertibleDocument=file=>isDocx(file)||isGoogleDoc(file);

async function listChildren(parent,pageSize=1000){
  const files=[];let pageToken='';
  do{
    const url=new URL('https://www.googleapis.com/drive/v3/files');url.searchParams.set('q',`'${parent}' in parents and trashed=false`);url.searchParams.set('fields','nextPageToken,files(id,name,mimeType,createdTime,modifiedTime,size,parents)');url.searchParams.set('pageSize',String(Math.max(1,Math.min(1000,pageSize))));url.searchParams.set('orderBy','createdTime desc');url.searchParams.set('supportsAllDrives','true');url.searchParams.set('includeItemsFromAllDrives','true');if(pageToken)url.searchParams.set('pageToken',pageToken);
    const response=await driveFetch(url,{headers:{accept:'application/json'}});if(!response.ok)throw new Error(`Google Drive list ${response.status}`);const payload=await response.json();if(Array.isArray(payload?.files))files.push(...payload.files);pageToken=String(payload?.nextPageToken||'');
  }while(pageToken);
  return files;
}

async function collectSubjectDocuments(folder,subject,visited){
  const folderId=String(folder?.id||'');if(!folderId||visited.has(folderId))return[];visited.add(folderId);
  const rows=await listChildren(folderId,1000),files=rows.filter(isConvertibleDocument).map(file=>({...file,parentName:subject,parentFolderId:folderId})),childFolders=rows.filter(row=>row?.mimeType===FOLDER_MIME&&clean(row?.name,160));
  const nested=await Promise.all(childFolders.map(child=>collectSubjectDocuments(child,subject,visited)));
...
async function downloadAsDocx(file){
  const native=isGoogleDoc(file),url=new URL(native?`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(file.id)}/export`:`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(file.id)}`);if(native)url.searchParams.set('mimeType',DOCX_MIME);else url.searchParams.set('alt','media');
  const response=await driveFetch(url,{},15000);if(!response.ok)throw new Error(native?`Google Docs export ${response.status}`:`Google Drive download ${response.status}`);const len=Number(response.headers.get('content-length')||file.size||0);if(len>MAX_DRIVE_BYTES)throw new Error('Tài liệu vượt 5 MB');const buffer=Buffer.from(await response.arrayBuffer());if(!buffer.length||buffer.length>MAX_DRIVE_BYTES)throw new Error('Tài liệu không hợp lệ hoặc vượt 5 MB');return buffer;
}
...
const documentMeta=(file,sourceHash,questionCount,syncStatus='ready',syncMessage='')=>({driveFileId:String(file.id),fileName:clean(file.name,300),mimeType:clean(file?.mimeType||DOCX_MIME,180),modifiedTime:file.modifiedTime||null,sourceHash,subjectHint:sourceSubject(file),syncStatus,syncMessage:versionedMessage(syncMessage||`Đã nhận ${questionCount} câu có đúng một đáp án tô đỏ từ nguồn Word/Google Docs.`)});
...
export async function handleTrustedQuizIngest(req,res){
  ...
  try{
    ...
    for(const file of selected){try{processed.push(await importRedAnswerDocx(req,file,await downloadAsDocx(file)))}catch(error){processed.push({ok:false,status:'error',fileName:clean(file.name,300),subject:sourceSubject(file),message:clean(error?.message||'Không xử lý được tài liệu',300)})}}
