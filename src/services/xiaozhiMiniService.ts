...
export async function askXiaoZhiMini(query:string,localContext=''):Promise<XiaoZhiReply>{
  ...
  const response=await fetch('/api/ai/assistant',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({mode:'xiaozhi-mini',query,localContext,pageContext:`${location.pathname}${location.search}`})});
