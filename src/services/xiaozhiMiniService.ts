...
export async function askXiaoZhiMini(query:string,localContext=''):Promise<XiaoZhiReply>{
  ...
  const response=await fetch('/api/ai/xiaozhi-mini',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({query,localContext,pageContext:`${location.pathname}${location.search}`})});
