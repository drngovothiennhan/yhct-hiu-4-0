...
export async function askAcademicUnified(rawQuery:string,localWorks:ResearchWork[]=[]):Promise<AcademicAiReply>{
  ...
  const result=await askServerAi(`Bạn là trợ lý học thuật Y học cổ truyền cho sinh viên HIU. Trả lời câu hỏi sau dựa trên nguồn đã truy xuất, phân biệt kiến thức kinh điển với bằng chứng hiện đại khi cần, không bịa nguồn và không thay thế chẩn đoán/điều trị trực tiếp cho người bệnh: ${query}`,'research',sources);
  ...
  return{answer:result.degraded?buildAcademicFallback(query,evidence):renderAiAnswer(result),sources:links,provider:result.provider,degraded:result.degraded,latencyMs:result.latencyMs,provenance};
}
