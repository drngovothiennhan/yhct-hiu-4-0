...
export async function quizWorkspace<T>(action:string,data:Record<string,unknown>={}):Promise<T>{
 ...
 try{const r=await fetch('/api/ai/drive-rag',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({action,...data}),signal:controller.signal});const result=await r.json().catch(()=>({}));if(!r.ok){const error=new Error(result.error||result.message||`Lỗi nhập liệu ${r.status}`) as QuizWorkspaceError;error.status=r.status;if(result.draft)error.draft=result.draft as QuizDraft;throw error}return result as T}finally{clearTimeout(timer)}
}
const emit=async(update:QuizPipelineUpdate|undefined,draft:QuizDraft)=>{if(update)await update(draft)};
export async function continueQuizPipeline(initial:QuizDraft,onUpdate?:QuizPipelineUpdate):Promise<QuizDraft>{let current=initial;await emit(onUpdate,current);while(current.pipeline?.state==='processing'){try{current=await quizWorkspace<QuizDraft>('quiz-process-chunk',{id:current.id,revision:current.revision})}catch(error){const known=(error as QuizWorkspaceError).draft;current=known||await quizWorkspace<QuizDraft>('quiz-draft',{id:current.id});await emit(onUpdate,current);throw error}await emit(onUpdate,current)}if(current.pipeline?.state==='error'){const error=new Error(current.pipeline.lastError||'Pipeline đang dừng và có thể thử lại.') as QuizWorkspaceError;error.draft=current;throw error}return current}
...
export async function syncQuizBank(maxFiles=10):Promise<TrustedQuizSyncResult>{
 ...
 while(rounds++<120){const part=await quizWorkspace<TrustedQuizSyncResult>('trusted-quiz-sync',{maxFiles:batch});latest=part;if(firstPending===undefined)firstPending=Number(part.pending||0);processed.push(...(part.processed||[]));const hasTransportError=(part.processed||[]).some(x=>x.status==='error');if(hasTransportError||!Number(part.remaining||0)||!(part.processed||[]).length)break}
...
// Legacy conversion helpers remain backend-compatible but are intentionally not mounted by the canonical ACC bank UI.
