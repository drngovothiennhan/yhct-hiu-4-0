...
export type QuizDraft={id:string;revision:number;questions:QuizCandidate[];total:number;ready:number;needsReview:number;warnings:string[];unassigned:string[];sourceText:string;document:{fileName:string;subjectHint:string};file:{id:string;name:string;parentName:string}};
export type QuizDraftSummary={id:string;fileName:string;subject:string;total:number;pending:number;updatedAt:string};
export async function quizWorkspace<T>(action:string,data:Record<string,unknown>={}):Promise<T>{
 ...
 try{const r=await fetch('/api/ai/drive-rag',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({action,...data}),signal:controller.signal});const result=await r.json().catch(()=>({}));if(!r.ok)throw new Error(result.error||`Lỗi nhập liệu ${r.status}`);return result as T}finally{clearTimeout(timer)}
}
