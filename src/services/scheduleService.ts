...
export async function fetchSchedules():Promise<ActivitySchedule[]>{try{const {data,error}=await supabase.rpc('schedule_feed_v1');if(error)throw error;const items:ActivitySchedule[]=(Array.isArray(data)?data:[]).map(map);save(items);return items}catch{return loadCachedSchedules()}}
