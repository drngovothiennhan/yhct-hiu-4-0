...
export async function getDailyPracticeConfig():Promise<DailyPracticeConfig>{
  const {data,error}=await supabase.rpc('daily_practice_config_v1');if(error)throw error;return data as DailyPracticeConfig;
}
export async function getTodayDailyPractice(count=10):Promise<DailyPracticeSession>{
  const {data,error}=await supabase.rpc('daily_practice_today_v1',{p_count:Math.max(1,Math.min(30,Math.trunc(count)||10))});if(error)throw error;return data as DailyPracticeSession;
}
export async function answerDailyPractice(sessionId:string,questionId:string,selectedIndex:number):Promise<DailyPracticeAnswer>{
