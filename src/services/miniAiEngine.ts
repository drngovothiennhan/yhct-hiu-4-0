...
function openDb():Promise<IDBDatabase>{
  return new Promise((resolve,reject)=>{
    if(typeof indexedDB==='undefined'){reject(new Error('IndexedDB không khả dụng.'));return}
    const request=indexedDB.open(DB_NAME,DB_VERSION);
    request.onupgradeneeded=()=>{
      const db=request.result;
      if(!db.objectStoreNames.contains(STORE)){const store=db.createObjectStore(STORE,{keyPath:'id'});store.createIndex('kind','kind',{unique:false});store.createIndex('name','name',{unique:false})}
...
function waitTransaction(tx:IDBTransaction){return new Promise<void>((resolve,reject)=>{tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error||new Error('IndexedDB transaction lỗi.'));tx.onabort=()=>reject(tx.error||new Error('IndexedDB transaction bị hủy.'))})}
async function seedChunk(key:string,loader:()=>Promise<KnowledgeRecord[]>){
  const db=await openDb();
  try{
    const known=await new Promise<boolean>((resolve,reject)=>{const req=db.transaction(META,'readonly').objectStore(META).get(key);req.onsuccess=()=>resolve(Boolean(req.result?.ready));req.onerror=()=>reject(req.error)});
    if(known)return;
    const rows=await loader(),tx=db.transaction([STORE,META],'readwrite'),store=tx.objectStore(STORE);
...
function recordText(record:KnowledgeRecord){return normalize([record.name,...(record.aliases||[]),asText(record.code),asText(record.category),asText(record.actions),asText(record.indications),asText(record.meridian),asText(record.commonUses)].filter(Boolean).join(' '))}
function localScore(query:string,record:KnowledgeRecord){
  const q=normalize(query),text=recordText(record);if(!q)return 0;if(text===q)return 1;if(text.startsWith(q)||normalize(record.name).startsWith(q))return .96;if(text.includes(q))return .88;
  const qTokens=tokens(q);if(!qTokens.length)return 0;let matches=0;for(const token of qTokens)if(text.includes(token))matches++;
  const tokenScore=matches/qTokens.length,name=normalize(record.name),prefix=qTokens.some(token=>name.startsWith(token))?.08:0;
  return Math.min(.86,tokenScore*.78+prefix);
}
function filterAllows(filter:OmniFilter,record:KnowledgeRecord){return filter==='all'||(filter==='medicine'&&(record.kind==='herb'||record.kind==='formula'))||(filter==='acupoint'&&record.kind==='acupoint')}
export async function searchKnowledge(query:string,filter:OmniFilter='all',limit=8):Promise<KnowledgeHit[]>{
  const started=performance.now();await ensureKnowledgeBase();const db=await openDb();
  try{
    const records=await new Promise<KnowledgeRecord[]>((resolve,reject)=>{const req=db.transaction(STORE,'readonly').objectStore(STORE).getAll();req.onsuccess=()=>resolve((req.result||[]) as KnowledgeRecord[]);req.onerror=()=>reject(req.error)});
    const ranked=records.filter(record=>filterAllows(filter,record)).map(record=>({record,score:localScore(query,record)})).filter(item=>item.score>=.2).sort((a,b)=>b.score-a.score).slice(0,Math.max(1,Math.min(limit,20)));
    const elapsedMs=Math.round((performance.now()-started)*10)/10;return ranked.map(item=>({...item,elapsedMs}));
  }finally{db.close()}
}

export function buildResearchLinks(topic:string):ResearchLink[]{
  const clean=topic.trim().replace(/\s+/g,' ');if(!clean)return[];
  const scholar=`${clean} traditional medicine`,pubmed=`(${clean}) AND (traditional medicine OR acupuncture OR herbal medicine)`,cochrane=`${clean} traditional medicine`,pdf=`${clean} "y học cổ truyền" filetype:pdf (site:edu.vn OR site:gov.vn OR site:who.int)`;
  return[
...
export function parseDrlIntent(text:string){
  ...
  if(/^(xem\s+)?diem(\s+ren\s+luyen)?(\s+ky\s+nay)?\s+cua\s+(toi|minh)$/.test(normalized)||/diem\s+ky\s+nay\s+cua\s+(toi|minh)/.test(normalized))return{mode:'mine' as const,query:''};
  const match=normalized.match(/(?:xem\s+)?diem(?:\s+ren\s+luyen)?\s+cua\s+(.{2,60})$/);if(match)return{mode:'public' as const,query:match[1].trim()};
  const code=text.match(/\b\d{6,14}\b/);if(code&&normalized.includes('diem'))return{mode:'public' as const,query:code[0]};return null;
}
export async function checkDrlConversation(text:string,member:Member|null):Promise<DrlSummary|null>{
  ...
  if(intent.mode==='mine'){
    ...
    const {data,error}=await supabase.rpc('drl_member_history_v1');if(error)throw error;const rows=(Array.isArray(data)?data:[]) as DrlHistoryRow[];
    ...
    for(const row of rows){const group=groups.get(row.semester_code)||{title:row.semester_title,total:0,items:[]};group.total+=Number(row.points||0);group.items.push(row);groups.set(row.semester_code,group)}
    const current=[...groups.values()].sort((a,b)=>{const ad=Math.max(...a.items.map(x=>Date.parse(x.occurred_at||x.created_at)||0)),bd=Math.max(...b.items.map(x=>Date.parse(x.occurred_at||x.created_at)||0));return bd-ad})[0];
    return{mode:'mine',query:'',semesterTitle:current.title,total:current.total,items:current.items.map(row=>({label:row.activity_name,detail:row.note||new Date(row.occurred_at||row.created_at).toLocaleDateString('vi-VN'),points:Number(row.points||0)}))};
  }
  const {data,error}=await supabase.rpc('drl_public_search_v1',{p_query:intent.query,p_limit:20});if(error)throw error;const candidates=rankDrlCandidates(intent.query,(data||[]) as DrlPublicRow[]).slice(0,6);
...
function parsePoints(text:string){const match=text.match(/(?:\+\s*)?(\d{1,3})\s*(?:điểm|diem)\b/i);if(!match)return null;const value=Number(match[1]);return Number.isFinite(value)&&value>=0&&value<=200?value:null}
...
export async function getDrlReminders(member:Member|null,horizonDays=30):Promise<{items:DrlReminder[];target:number|null;current:number|null;gap:number|null}>{
  let schedules=loadCachedSchedules();if(!schedules.length){try{schedules=await fetchSchedules()}catch{schedules=[]}}
  const items=schedules.filter((item:ActivitySchedule)=>item.status==='scheduled'&&daysUntil(item.startsAt)>=0&&daysUntil(item.startsAt)<=horizonDays&&(!member||item.visibility!=='assignees'||item.assigneeIds.includes(member.id))).sort((a,b)=>Date.parse(a.startsAt)-Date.parse(b.startsAt)).slice(0,8).map(item=>({id:item.id,title:item.title,startsAt:item.startsAt,daysLeft:daysUntil(item.startsAt),points:parsePoints(`${item.title} ${item.notes}`),location:item.location,note:item.notes}));
  let target:number|null=null;try{const parsed:unknown=JSON.parse(localStorage.getItem(DRL_TARGET_KEY)||'null'),raw=typeof parsed==='object'&&parsed!==null&&'target' in parsed?(parsed as {target:unknown}).target:parsed,value=Number(raw);if(Number.isFinite(value)&&value>0&&value<=200)target=value}catch{}
  let current:number|null=null;if(member){try{current=(await checkDrlConversation('Điểm kỳ này của tôi',member))?.total??null}catch{current=null}}
  return{items,target,current,gap:target!==null&&current!==null?Math.max(0,target-current):null};
}

const SOLAR_TERMS:[number,number,string][]=[[1,5,'Tiểu hàn'],[1,20,'Đại hàn'],[2,4,'Lập xuân'],[2,19,'Vũ thủy'],[3,5,'Kinh trập'],[3,20,'Xuân phân'],[4,4,'Thanh minh'],[4,20,'Cốc vũ'],[5,5,'Lập hạ'],[5,21,'Tiểu mãn'],[6,5,'Mang chủng'],[6,21,'Hạ chí'],[7,7,'Tiểu thử'],[7,22,'Đại thử'],[8,7,'Lập thu'],[8,23,'Xử thử'],[9,7,'Bạch lộ'],[9,23,'Thu phân'],[10,8,'Hàn lộ'],[10,23,'Sương giáng'],[11,7,'Lập đông'],[11,22,'Tiểu tuyết'],[12,7,'Đại tuyết'],[12,21,'Đông chí']];
function getSolarTerm(date:Date){const md=(date.getMonth()+1)*100+date.getDate();let current=SOLAR_TERMS[SOLAR_TERMS.length-1];for(const term of SOLAR_TERMS){if(term[0]*100+term[1]<=md)current=term;else break}return current[2]}
function getSeason(month:number){if(month>=2&&month<=4)return'Xuân';if(month>=5&&month<=7)return'Hạ';if(month>=8&&month<=10)return'Thu';return'Đông'}
export function generateDailyContent(date=new Date()):DailyContentPack{
  const term=getSolarTerm(date),season=getSeason(date.getMonth()+1),dateLabel=date.toLocaleDateString('vi-VN',{weekday:'long',day:'2-digit',month:'2-digit',year:'numeric'}),base=`${season} · ${term}`;
...
export function describeKnowledge(record:KnowledgeRecord){
  if(record.kind==='formula')return[{k:'Thành phần',v:asText(record.components)},{k:'Liều tham khảo',v:asText(record.referenceDose)},{k:'Công năng',v:asText(record.actions)},{k:'Chủ trị học tập',v:asText(record.indications)},{k:'Thận trọng',v:asText(record.cautions)}];
  if(record.kind==='herb')return[{k:'Nhóm',v:asText(record.category)},{k:'Tính vị',v:asText(record.natureFlavor)},{k:'Quy kinh',v:asText(record.meridians)},{k:'Công năng',v:asText(record.actions)},{k:'Liều tham khảo',v:asText(record.referenceDose)},{k:'Thận trọng',v:asText(record.cautions)}];
  return[{k:'Mã huyệt',v:asText(record.code)},{k:'Kinh',v:asText(record.meridian)},{k:'Vị trí',v:asText(record.location)},{k:'Cách xác định',v:asText(record.howToLocate)},{k:'Ứng dụng học tập',v:asText(record.commonUses)},{k:'An toàn',v:asText(record.safeUse)}];
}
