...
function getStoredKey(){return safeJson<StoredKey|null>(localStorage.getItem(KEY_STORAGE),null)?.key?.trim()||''}
export function maskGeminiKey(key=getStoredKey()){return key?`••••••${key.slice(-4)}`:''}
export function saveGeminiKey(key:string){const clean=key.trim();if(!clean){localStorage.removeItem(KEY_STORAGE);localStorage.removeItem(MODEL_STORAGE);return}if(clean.length<20)throw new Error('API key không hợp lệ hoặc quá ngắn.');localStorage.setItem(KEY_STORAGE,JSON.stringify({key:clean,savedAt:new Date().toISOString()} satisfies StoredKey));localStorage.removeItem(MODEL_STORAGE)}
export function clearGeminiKey(){localStorage.removeItem(KEY_STORAGE);localStorage.removeItem(MODEL_STORAGE);localStorage.removeItem(QUOTA_STORAGE)}
...
export async function discoverGeminiModel(force=false){
  ...
  const cached=safeJson<StoredModel|null>(localStorage.getItem(MODEL_STORAGE),null);if(!force&&cached?.name&&Date.now()-cached.savedAt<MODEL_CACHE_MS)return cached.name;
  ...
  localStorage.setItem(MODEL_STORAGE,JSON.stringify({name:model,savedAt:Date.now()} satisfies StoredModel));return model;
}
...
export function getGeminiStatus():GeminiStatus{
  const key=getStoredKey(),model=safeJson<StoredModel|null>(localStorage.getItem(MODEL_STORAGE),null)?.name||null,q=readQuota();
