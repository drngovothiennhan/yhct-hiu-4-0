...
export function readReviewCards(identity:string|null):ReviewCard[]{
  try{const data=JSON.parse(localStorage.getItem(key(identity))||'[]');return Array.isArray(data)?data.filter((c:ReviewCard)=>c&&typeof c.id==='string'&&typeof c.topic==='string'&&typeof c.stem==='string'&&typeof c.answer==='string'&&typeof c.source==='string'&&(c.subject===undefined||typeof c.subject==='string')&&Number.isFinite(c.due)&&Number.isFinite(c.interval)&&c.interval>=0&&Number.isInteger(c.streak)&&c.streak>=0).slice(0,500):[]}catch{return []}
}
