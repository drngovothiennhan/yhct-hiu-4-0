...
export type ModuleId='feed'|'library'|'research'|'profile'|'garden'|'notifications'|'schedule'|'drl'|'exam'|'admin'|'acc';
...
export const MODULES:Record<ModuleId,ModuleDefinition>={
  ...
  library:{id:'library',title:'Thư viện học tập',path:'/library',label:'Thư viện',memberOnly:false,featureScope:'learning-resource-library'},
  ...
  exam:{id:'exam',title:'Learning Hub',path:'/exam',label:'Học',memberOnly:false,featureScope:'exam-center'},
...
export const MODULE_ORDER:ModuleId[]=['feed','exam','library','research','profile','garden','notifications','schedule','drl','admin','acc'];
...
export function moduleFromPath(pathname:string):ModuleId{const path=normalizeModulePath(pathname);if(path==='/messages')return'profile';if(/^\/r\/hiu_res_[0-9a-f]{20}$/.test(path))return'library';return PATH_TO_MODULE.get(path)||'feed'}
