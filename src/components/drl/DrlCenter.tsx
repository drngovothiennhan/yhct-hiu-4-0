...
export default function DrlCenter({member}:{member:Member|null}){
  ...
  const createDetectedSemester=async()=>{
    ...
    try{
      ...
      setResolvedSemester(matched.code);setPreview(current=>current.map(r=>({...r,hoc_ky:matched.code})));setMsg(`Đã tạo và ánh xạ ${matched.title} (${matched.code}). File sẵn sàng để ghi nhận.`);
  ...
  const commit=async()=>{
    ...
    if(structuredImport&&!resolvedSemester){setMsg('File đã được nhận diện nhưng chưa có học kỳ tương ứng. Hãy tạo/ánh xạ học kỳ trước khi ghi nhận.');return}
    setBusy(true);setMsg('');
    try{
      ...
      const accepted=Number(x.accepted||0),serverRejected=Number(x.rejected||0),duplicates=Number(x.duplicates||0),totalRejected=clientRejected+serverRejected;
      setMsg(`Đã upload và ghi nhận ${accepted.toLocaleString('vi-VN')} hồ sơ điểm rèn luyện hợp lệ. ${totalRejected.toLocaleString('vi-VN')} dòng không hợp lệ/không khớp đã bị loại · ${duplicates.toLocaleString('vi-VN')} dòng trùng được bỏ qua. Dữ liệu đã lưu và có thể tra cứu theo MSSV; học kỳ chưa công bố vẫn chỉ hiện “Đang tổng hợp / Chờ duyệt”.`);
