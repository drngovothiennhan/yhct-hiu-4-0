...
type ExactSemester={student_code_masked:string;semester_code:string;semester_title:string;is_published:boolean;publication_status:'published'|'recording'|'waiting';total_points:number|null;activities:ExactActivity[]};
...
export default function DrlCenter({member}:{member:Member|null}){
  ...
  const commit=async()=>{
    ...
    try{
      ...
      const x=(data||{}) as ImportResult;await reloadSemesters();await reloadHistory().catch(()=>{});
      const code=q.trim().replace(/\s/g,'').toUpperCase();if(MSSV.test(code))await lookupMssv(code).catch(()=>{});
      ...
      setMsg(`Đã upload và ghi nhận ${accepted.toLocaleString('vi-VN')} hồ sơ điểm rèn luyện hợp lệ. ${totalRejected.toLocaleString('vi-VN')} dòng không hợp lệ/không khớp đã bị loại · ${duplicates.toLocaleString('vi-VN')} dòng trùng được bỏ qua. Điểm hoạt động đã được cộng và có thể tra cứu ngay theo MSSV.${details?` Chi tiết: ${details}.`:''}`);
  ...
  return <section>
    ...
    <section className="panel"><h3>Tra cứu DRL</h3><div className="searchbox"><Search/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Nhập MSSV để xem trạng thái/điểm; hoặc họ tên để tìm điểm đã công bố" autoCapitalize="characters"/></div><small className="muted">Mỗi điểm hoạt động được cộng ngay sau khi Admin ghi nhận. Thành viên tra cứu đúng MSSV của mình sẽ thấy điểm đang cộng; công bố dùng để xác nhận tổng điểm học kỳ.</small>
      {exactMode&&<div className="drl-public-semesters">{exactRows.map(semester=><article className={`drl-public-semester ${semester.is_published?'is-published':'is-waiting'}`} key={semester.semester_code}><header><div><b>{semester.semester_title}</b><small>{semester.student_code_masked} · {semester.semester_code}</small></div><span className="drl-public-status">{semester.is_published?<><CheckCircle2/>Đã chốt điểm</>:semester.publication_status==='recording'?<>Đang cộng điểm</>:<>Đang tổng hợp / Chờ duyệt</>}</span></header>{semester.total_points!==null?<><strong className="drl-public-total">{Number(semester.total_points||0).toLocaleString('vi-VN')} điểm</strong><div className="data-list compact">{semester.activities.map(activity=><article key={activity.id}><div><b>{activity.activity_name}</b><small>{activity.note||'Không ghi chú'}{activity.occurred_at?` · ${new Date(activity.occurred_at).toLocaleDateString('vi-VN')}`:''}</small></div><strong>{Number(activity.points)>0?'+':''}{Number(activity.points)}</strong></article>)}</div></>:<p className="muted">Học kỳ chưa công bố. Điểm đang cộng chỉ hiển thị khi thành viên tra cứu đúng MSSV của chính mình.</p>}</article>)}{q.trim().length>=8&&exactRows.length===0&&<p className="muted">Chưa có dữ liệu hoạt động phù hợp với MSSV này.</p>}</div>}
    ...
    {canManage&&<section className="panel"><div className="row panel-title"><ShieldCheck/><div><h3>Quản trị DRL · Nhập hoạt động → Công bố</h3><p>Mỗi hoạt động nhập bằng file tương ứng · điểm cộng ngay cho thành viên · công bố xác nhận tổng điểm học kỳ và chỉ dành cho Admin.</p></div></div><label className="upload-box"><FileSpreadsheet/><span>Chọn Excel/CSV đúng mẫu HIU · tối đa 10 MB / 5.000 dòng</span><input type="file" accept=".xlsx,.xls,.csv" disabled={busy} onChange={e=>{const f=e.target.files?.[0];if(f)void parseFile(f);e.currentTarget.value=''}}/></label>
      ...
      <div className="semester-grid">{semesters.map(s=><article key={s.id}><b>{s.title}</b><small>{s.code} · {Number(s.row_count||0).toLocaleString('vi-VN')} dòng/{Number(s.student_count||0).toLocaleString('vi-VN')} SV · {s.is_published?'Đã xác nhận tổng điểm':'Đang tổng hợp'} · {s.is_locked?'Đã khóa':'Đang mở'}</small><div className="schedule-actions">{canPublish&&<button className={s.is_published?'secondary':''} disabled={busy||(!s.is_published&&Number(s.row_count||0)===0)} onClick={()=>setPublishDialog({semester:s,target:!s.is_published,step:1})}><ShieldCheck/>{s.is_published?'Gỡ công bố':'Công bố điểm học kỳ'}</button>}{canLock&&<button className="secondary" disabled={busy} onClick={()=>void toggleLock(s)}><Lock/>{s.is_locked?'Mở khóa học kỳ':'Khóa sổ'}</button>}</div></article>)}</div></section>}

    {msg&&<div className="ai-note" role="status">{msg}</div>}

    {publishDialog&&<div className="modal drl-publish-backdrop" onMouseDown={()=>!busy&&setPublishDialog(null)}><section className="drl-publish-dialog" role="alertdialog" aria-modal="true" aria-labelledby="drl-publish-title" onMouseDown={e=>e.stopPropagation()}><button className="close" disabled={busy} onClick={()=>setPublishDialog(null)} aria-label="Đóng"><X/></button><ShieldCheck className="drl-publish-icon"/><h3 id="drl-publish-title">{publishDialog.target?'Công bố điểm học kỳ':'Gỡ công bố điểm'} · Bước {publishDialog.step}/2</h3>{publishDialog.step===1?<><p><b>{publishDialog.semester.title}</b> · {publishDialog.semester.code}</p><p>{publishDialog.target?'Công bố dùng để xác nhận tổng điểm học kỳ hiện tại. Điểm từng hoạt động đã ghi nhận trước đó vẫn được tra cứu.':'Gỡ công bố sẽ thu hồi xác nhận tổng điểm học kỳ; dữ liệu hoạt động đã ghi nhận không bị xóa.'}</p><button disabled={busy} onClick={()=>setPublishDialog(current=>current?{...current,step:2}:current)}>Tiếp tục xác nhận</button></>:<><div className="warning"><b>Xác nhận cuối cùng.</b><br/>{publishDialog.target?'Hệ thống sẽ xác nhận và công bố tổng điểm hiện tại của học kỳ này.':'Hệ thống sẽ gỡ trạng thái xác nhận tổng điểm của học kỳ này.'}</div><div className="schedule-actions"><button className="secondary" disabled={busy} onClick={()=>setPublishDialog(current=>current?{...current,step:1}:current)}>Quay lại</button><button disabled={busy} onClick={()=>void executePublication()}>{busy?'Đang xử lý…':publishDialog.target?'Xác nhận công bố':'Xác nhận gỡ công bố'}</button></div></>}</section></div>}
  </section>;
}
