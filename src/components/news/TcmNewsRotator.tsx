...
export default function TcmNewsRotator(){
  ...
  useEffect(()=>{
    ...
    const onWheel=(event:WheelEvent)=>{
      ...
      event.preventDefault();pending+=delta;if(!raf)raf=window.requestAnimationFrame(flush);
    };
  ...
  const onMouseDown=(event:React.MouseEvent<HTMLDivElement>)=>{
    ...
    dragRef.current={startX:event.clientX,startScrollLeft:trackRef.current.scrollLeft,moved:false};setDragging(true);setPaused(true);
  };
  const onMouseMove=(event:React.MouseEvent<HTMLDivElement>)=>{
    const drag=dragRef.current,track=trackRef.current;if(!drag||!track)return;
    const dx=event.clientX-drag.startX;if(!drag.moved&&Math.abs(dx)>DRAG_THRESHOLD_PX)drag.moved=true;if(!drag.moved)return;
    event.preventDefault();track.scrollLeft=drag.startScrollLeft-dx;
  };
  const finishMouseDrag=()=>{
    const drag=dragRef.current;if(!drag)return;
    dragRef.current=null;setDragging(false);setPaused(false);
    if(drag.moved){suppressClickRef.current=true;if(suppressTimerRef.current!==null)window.clearTimeout(suppressTimerRef.current);suppressTimerRef.current=window.setTimeout(()=>{suppressClickRef.current=false;suppressTimerRef.current=null},220)}
  };
  const onClickCapture=(event:React.MouseEvent<HTMLDivElement>)=>{
    if(!suppressClickRef.current)return;suppressClickRef.current=false;
  ...
  return <section className="news-rotator" aria-label="Điểm tin Y học cổ truyền tự động">
    <div className="news-rotator-heading"><div className="row panel-title"><HerbIcon/><div><h2>Tin tức Y học cổ truyền</h2><p>Tự tải và luân phiên · 2 tin mỗi lượt trên web, 1 tin mỗi lượt trên điện thoại.</p></div></div><button className="secondary news-rotator-refresh" disabled={busy} onClick={()=>void load()} aria-label="Làm mới tin"><RefreshCw/></button></div>
    ...
    {items.length===0?<div className="news-rotator-empty"><HerbIcon/><span>{busy?'Đang tải tin…':'Chưa có tin đã công bố.'}</span></div>:
      <div className="news-rotator-stage" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>{setPaused(false);finishMouseDrag()}} onFocusCapture={()=>setPaused(true)} onBlurCapture={()=>setPaused(false)}>
        ...
        <div ref={trackRef} className={`news-rotator-track${dragging?' is-dragging':''}`} onScroll={onScroll} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={finishMouseDrag} onClickCapture={onClickCapture}>
          {items.map(n=><a className="news-rotator-card" key={n.id} href={n.canonical_url} target="_blank" rel="noreferrer noopener" draggable={false}><div className="news-rotator-meta"><span>{n.publisher||n.publisher_domain||'Nguồn tổng hợp'}</span><time dateTime={n.published_at||undefined}>{shortDate(n.published_at)}</time></div><p className="news-rotator-text"><strong>{n.title}</strong>{n.summary?` — ${n.summary}`:' — Tóm tắt đang được cập nhật.'}</p><span className="news-rotator-open">Đọc nguồn <ExternalLink/></span></a>)}
        </div>
