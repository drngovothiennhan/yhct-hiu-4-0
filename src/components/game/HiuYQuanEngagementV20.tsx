...
class V20EngagementBoundary extends Component<BoundaryProps,BoundaryState>{
  state:BoundaryState={failed:false};
  static getDerivedStateFromError(){return {failed:true}}
  render(){return this.state.failed?<section className="hyq-v15-fallback" role="status"><b>Thử thách học tập tạm thời chưa khả dụng.</b><span>HIU Y Quán V20 vẫn hoạt động bình thường. Hãy tiếp tục khám bệnh và thử lại sau.</span></section>:this.props.children}
}
...
function Inner({member,onClinicRefresh}:Props){
  ...
  const loadEngagement=async(silent=false)=>{try{const data=await safeCall<Engagement>('hiu_y_quan_engagement_v15');setEngagement(data);if(!silent)setMessage('')}catch(e){if(!silent)setMessage((e as Error).message||'Không tải được tiến độ học tập.')}};
  ...
  const refreshAll=async()=>{if(busy)return;setBusy('refresh');await Promise.allSettled([loadEngagement(true),loadConsult(true),loadCollection(true)]);setBusy('');setMessage('Đã đồng bộ tiến độ HIU Y Quán với máy chủ.')};

  return <section className="hyq-v15" aria-label="HIU Y Quán V20 tương tác hằng ngày">
    <header className="hyq-v15-head"><div><span className="hyq-v15-kicker"><Zap/> V20 · ENGAGEMENT</span><h3>Thử thách học tập HIU Y Quán</h3><p>XP và mastery tách biệt tín dụng Gia Viên. Tất cả ca, hội chẩn và thử thách là mô phỏng giáo dục.</p></div><button type="button" className="hyq-v15-refresh" disabled={Boolean(busy)} onClick={()=>void refreshAll()}><RefreshCw/>Đồng bộ</button></header>
    ...
    <nav className="hyq-v15-tabs" role="tablist" aria-label="Các thử thách HIU Y Quán">{tabs.map(item=><button key={item.id} role="tab" aria-selected={tab===item.id} className={tab===item.id?'active':''} onClick={()=>setTab(item.id)}><b>{item.label}</b><small>{item.sub}</small></button>)}</nav>
    ...
    {tab==='collection'&&<section className="hyq-v15-panel hyq-v15-collection"><header><div><Award/><span><b>Bộ sưu tập mastery</b><small>Thẻ học tập và danh hiệu HIU Y Quán · không ghi vào kho Gia Viên</small></span></div><strong>{collection?.title||engagement?.title||'Học đồ HIU'}</strong></header>{!collection?<div className="hyq-v15-empty"><RefreshCw/><b>Đang tải bộ sưu tập…</b></div>:<><h4>Dược liệu</h4><div className="hyq-v15-card-grid">{collection.herbs.map(card=><article key={card.key} className={card.unlocked?'unlocked':'locked'}><span>{tierMarks[card.tier]||card.tier}</span><b>{card.name}</b><small>{card.badge} · {card.points} mastery</small></article>)}</div><h4>Thể bệnh</h4><div className="hyq-v15-card-grid syndrome">{collection.syndromes.map(card=><article key={card.key} className={card.unlocked?'unlocked':'locked'}><span>{tierMarks[card.tier]||card.tier}</span><b>{card.name}</b><small>{card.badge} · {card.points} mastery</small></article>)}</div></>}</section>}

    <footer className="hyq-v15-safety">HIU Y Quán V20 là mô phỏng học tập. Không sử dụng kết quả game để chẩn đoán, kê đơn hoặc quyết định xử trí cho người bệnh thực tế.</footer>
  </section>;
}

export default function HiuYQuanEngagementV20(props:Props){return <V20EngagementBoundary><Inner {...props}/></V20EngagementBoundary>}
