...
type Quota={allowed:boolean;unlimited?:boolean;limit:number|null;remaining:number|null;used:number;windowHours:number;retryAt?:string|null;role?:string};
...
export default function ResearchProposalBuilder({member,works,initialTitle='',onClose}:Props){
  ...
  const exhausted=Boolean(quota&&!quota.unlimited&&(quota.remaining??0)<=0);

  const geminiAssist=async()=>{
    if(clean(form.title).length<5){setNote('Hãy nhập tên đề tài trước khi dùng Gemini.');return}
    if(exhausted){setNote(`Đã dùng đủ ${quota?.limit??0} lượt Gemini trong chu kỳ ${quota?.windowHours??6} giờ. Hãy chỉnh sửa bản hiện có hoặc chờ chu kỳ mới.`);return}
    ...
    try{
      ...
      setForm(f=>({...f,...payload.draft}));
      const quotaText=payload.quota?.unlimited?'Admin · không giới hạn lượt đề cương':`Còn ${payload.quota?.remaining??'—'}/${payload.quota?.limit??'—'} lượt trong chu kỳ ${payload.quota?.windowHours??6} giờ`;
      setNote(`Đã tạo đề cương bằng Gemini Research${payload.model?` · ${payload.model}`:''}. ${quotaText}. Kiểm chứng nguồn và xin GVHD duyệt trước khi nộp.`);
  ...
  const quotaText=!quota?'Đang kiểm tra hạn mức…':quota.unlimited?'Admin · không giới hạn lượt tạo đề cương':`${quota.remaining??0}/${quota.limit??0} lượt Gemini còn lại · ${quota.windowHours} giờ/chu kỳ${(quota.remaining??0)===0&&retryText?` · mở lại khoảng ${retryText}`:''}`;
  ...
  return <div className="research-proposal-backdrop" onMouseDown={onClose}><section className="research-proposal" role="dialog" aria-modal="true" aria-label="Tạo đề cương nghiên cứu" onMouseDown={e=>e.stopPropagation()}>
    ...
    <div className="proposal-toolbar"><label>Thiết kế nghiên cứu<select value={design} onChange={e=>setDesign(e.target.value)}>{DESIGNS.map(x=><option key={x}>{x}</option>)}</select></label><div className="proposal-actions"><button className="secondary" disabled={busy||exhausted} onClick={()=>void geminiAssist()}><Bot/>{busy?'Gemini đang tạo…':'Gemini tạo đề cương'}</button><button className="proposal-download" onClick={exportDocx}><Download/> Xuất Word</button></div></div>
    <div className="ai-note proposal-quota"><b><Clock3/> Hạn mức theo vai trò · chu kỳ 6 giờ</b><span>{quotaText}</span></div>
