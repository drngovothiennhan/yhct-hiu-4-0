...
export default function ResearchCenter({member,onLogin}:Props){
  ...
  const search=async()=>{
    ...
    setBusy(true);setErr('');
    try{
      const results=await Promise.allSettled([searchPubMed(query,12),searchOpenAlex(query,12),searchClinicalTrials(query,8)]),merged=dedupeWorks(results.flatMap(x=>x.status==='fulfilled'?x.value:[]));setWorks(merged);
