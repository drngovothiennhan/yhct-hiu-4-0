...
export default function ResearchAiMini({member,works,query,onOpenProposal}:Props){
  ...
  const ask=async()=>{const text=clean(input||query);if(!text||busy)return;setBusy(true);setNote('');try{
    const [drive,liveOpenAlex]=await Promise.all([searchDriveRag(text,4),searchOpenAlex(text,6).catch(()=>[])]),evidence=dedupe([...liveOpenAlex,...works]).slice(0,10),literature=asSources(evidence),sources=[...drive.sources,...literature].slice(0,6);
    if(sources.length){const result=await askServerAi(text,'research',sources);setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));const provenance=[drive.sources.length?`Drive ${drive.sources.length}`:'',liveOpenAlex.length?`OpenAlex ${liveOpenAlex.length}`:'',result.provider].filter(Boolean).join(' · ');setNote(result.degraded?`Nguồn đã truy xuất: ${provenance}. Cloud A.I degraded nên dùng fallback an toàn.`:`Nguồn: ${provenance} · ${Math.round(result.latencyMs)} ms`)}
    else{const result=await askServerAi(text,'research',[]);setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));setNote(result.degraded?'Drive/OpenAlex chưa có nguồn phù hợp; fallback local.':'A.I cloud chưa có nguồn truy xuất trong lượt này; cần kiểm chứng.')}
  ...
  return <div className={`research-ai-mini ${open?'is-open':''}`}>
    {open&&<section className="research-ai-mini-panel" role="dialog" aria-label="Trợ lý A.I học thuật"><header><div><b><Bot/> Research A.I Mini</b><small>{member.herbalAlias||member.fullName} · thành viên</small></div><button className="icon-btn" onClick={()=>setOpen(false)} aria-label="Đóng"><X/></button></header>
      <div className="research-ai-status"><Sparkles/><span><b>Cloud + Drive RAG + OpenAlex</b><small>Drive: kho nghiên cứu server-side · Translator local: {capabilities.translator?'sẵn sàng':'fallback API miễn phí'}</small></span></div>
