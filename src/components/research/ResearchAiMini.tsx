...
export default function ResearchAiMini({member,works,query,onOpenProposal}:Props){
  ...
  const ask=async()=>{const text=clean(input||query);if(!text||busy)return;setBusy(true);setNote('');try{
    ...
    else{const result=await askServerAi(text,'research',[]);setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));setNote(result.degraded?'Drive/OpenAlex chưa có nguồn phù hợp; dùng fallback local.':'A.I cloud chưa có nguồn truy xuất trong lượt này; cần kiểm chứng.')}
  ...
  return <div className={`research-ai-mini ${open?'is-open':''}`}>
    {open&&<section className="research-ai-mini-panel" role="dialog" aria-label="Trợ lý A.I học thuật"><header><div><b><Bot/> Research A.I Mini</b><small>{member.herbalAlias||member.fullName} · thành viên</small></div><button className="icon-btn" onClick={()=>setOpen(false)} aria-label="Đóng"><X/></button></header>
      <div className="research-ai-status"><Sparkles/><span><b>AI Platform · Cloud + Drive RAG + OpenAlex</b><small>Drive: kho nghiên cứu server-side · Translator local: {capabilities.translator?'sẵn sàng':'fallback API miễn phí'}</small></span></div>
