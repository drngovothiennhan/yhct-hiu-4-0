...
import {Bot,ChevronDown,Database,FileSearch,Languages,Lightbulb,Send,Sparkles,X} from 'lucide-react';
...
export default function ResearchAiMini({member,works,query,onOpenProposal}:Props){
  const [open,setOpen]=useState(false),[mode,setMode]=useState<Mode>('assistant'),[input,setInput]=useState(''),[answer,setAnswer]=useState(''),[busy,setBusy]=useState(false),[note,setNote]=useState(''),[target,setTarget]=useState<'vi'|'en'>('vi'),[useInternal,setUseInternal]=useState(false);
  const pending=useRef<AbortController|null>(null);
  const cancel=()=>{pending.current?.abort();pending.current=null;setBusy(false);setNote('Đã hủy yêu cầu.');};
  ...
  const ask=async()=>{const text=clean(input||query);if(!text||busy)return;const controller=new AbortController();pending.current=controller;setBusy(true);setNote('');try{
    const [drive,liveOpenAlex,central]=await Promise.all([useInternal?searchDriveRag(text,4,controller.signal):Promise.resolve({sources:[],degraded:false}),searchOpenAlex(text,6).catch(()=>[]),useInternal?searchKnowledge(text,'all',5).catch(()=>[]):Promise.resolve([])]),evidence=dedupe([...liveOpenAlex,...works]).slice(0,10),literature=asSources(evidence),knowledge=centralSources(central),sources=[...knowledge,...drive.sources,...literature].slice(0,6);
    if(controller.signal.aborted)return;
    if(sources.length){const result=await askServerAi(text,'research',sources,controller.signal,{useInternal});if(controller.signal.aborted)return;setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));const provenance=[central.length?`Central RAG ${central.length}`:'',drive.sources.length?`Drive ${drive.sources.length}`:'',liveOpenAlex.length?`OpenAlex ${liveOpenAlex.length}`:'',result.provider].filter(Boolean).join(' · ');setNote(result.degraded?`Nguồn đã truy xuất: ${provenance}. Cloud A.I đang chuyển chế độ nên dùng fallback học thuật có nguồn.`:`Nguồn: ${provenance} · ${Math.round(result.latencyMs)} ms`)}
    else{const result=await askServerAi(text,'research',[],controller.signal,{useInternal});if(controller.signal.aborted)return;setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));setNote(result.degraded?'Central RAG/OpenAlex chưa có nguồn phù hợp; dùng fallback local.':'Gemini chưa có nguồn truy xuất phù hợp trong lượt này; cần kiểm chứng.')}
  }catch(e){if(controller.signal.aborted)return;setAnswer(buildAcademicFallback(text,works));setNote((e as Error).message||'A.I học thuật chưa khả dụng; đã dùng fallback local.')}finally{if(pending.current===controller){pending.current=null;setBusy(false)}}};
  const translate=async()=>{const text=input.trim();if(!text||busy)return;const controller=new AbortController();pending.current=controller;setBusy(true);setNote('');try{const result=await translateAcademic(text,target);if(controller.signal.aborted)return;setAnswer(result.text);setNote(result.text?`Dịch qua ${result.provider}.`:(result.note||'Chưa dịch được nội dung.'))}catch(e){if(!controller.signal.aborted)setNote((e as Error).message)}finally{if(pending.current===controller){pending.current=null;setBusy(false)}}};
  return <div className={`research-ai-mini ${open?'is-open':''}`}>
    {open&&<section className="research-ai-mini-panel" role="dialog" aria-label="Trợ lý A.I học thuật"><header><div><b><Bot/> Research A.I Mini</b><small>{member.herbalAlias||member.fullName} · thành viên</small></div><button className="icon-btn" onClick={()=>{cancel();setOpen(false)}} aria-label="Đóng"><X/></button></header>
      ...
      <div className="research-ai-tabs"><button className={mode==='assistant'?'active':''} onClick={()=>{cancel();setMode('assistant');setAnswer('');setNote('')}}><Bot/> Học thuật</button><button className={mode==='translate'?'active':''} onClick={()=>{cancel();setMode('translate');setAnswer('');setNote('')}}><Languages/> Dịch</button><button className={mode==='topics'?'active':''} onClick={()=>{cancel();setMode('topics');setAnswer('');setNote('')}}><Lightbulb/> Đề tài</button></div>
      {mode==='assistant'&&<><label className="research-internal-toggle"><Database/><span><b>Dùng tài liệu nội bộ</b><small>{useInternal?'Gemini đối chiếu Drive và Central RAG':'Gemini chỉ tìm nguồn công khai'}</small></span><input type="checkbox" checked={useInternal} disabled={busy} onChange={event=>setUseInternal(event.target.checked)}/><i aria-hidden="true"/></label><textarea maxLength={4000} value={input} onChange={e=>setInput(e.target.value)} placeholder={query?`Hỏi sâu về: ${query}`:'Hỏi về y văn, phương pháp, khoảng trống nghiên cứu…'}/><button className="research-ai-primary" disabled={busy||!clean(input||query)} onClick={()=>void ask()}><Send/>{busy?`Gemini đang tìm ${useInternal?'Web + nội bộ':'nguồn công khai'}…`:'Hỏi Gemini học thuật'}</button></>}
      ...
      {busy&&<button className="secondary" onClick={cancel}>Hủy yêu cầu</button>}
