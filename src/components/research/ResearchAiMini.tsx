...
export default function ResearchAiMini({member,works,query,onOpenProposal}:Props){
  ...
  const sources=works.slice(0,6).map(w=>({id:`${w.provider}:${w.id}`,title:w.title,text:`${w.abstract||w.title} ${w.authors.join(', ')} ${w.source} ${w.year||''}`.slice(0,4200),url:w.url}));
  const ask=async()=>{const text=clean(input||query);if(!text||busy)return;setBusy(true);setNote('');try{if(sources.length){const result=await askServerAi(text,'research',sources);setAnswer(result.degraded?buildAcademicFallback(text,works):renderAiAnswer(result));setNote(result.degraded?'Đang dùng fallback local 0đ; A.I cloud chưa khả dụng.':`A.I học thuật có nguồn · ${result.provider} · ${Math.round(result.latencyMs)} ms`)}else{try{const result=await askServerAi(text,'research',[]);setAnswer(result.degraded?buildAcademicFallback(text,works):renderAiAnswer(result));setNote(result.degraded?'Chưa có nguồn y văn kèm theo; đang dùng fallback an toàn.':'A.I cloud đang trả lời không kèm nguồn phiên tìm kiếm; cần kiểm chứng tài liệu.')}catch{setAnswer(buildAcademicFallback(text,works));setNote('Fallback local 0đ.')}}}catch(e){setAnswer(buildAcademicFallback(text,works));setNote((e as Error).message||'A.I cloud chưa khả dụng; đã dùng fallback local.')}finally{setBusy(false)}};
  ...
  return <div className={`research-ai-mini ${open?'is-open':''}`}>
    {open&&<section className="research-ai-mini-panel" role="dialog" aria-label="Trợ lý A.I học thuật"><header><div><b><Bot/> Research A.I Mini</b><small>{member.herbalAlias||member.fullName} · thành viên</small></div><button className="icon-btn" onClick={()=>setOpen(false)} aria-label="Đóng"><X/></button></header>
      <div className="research-ai-status"><Sparkles/><span><b>A.I local-first</b><small>Translator local: {capabilities.translator?'sẵn sàng':'tùy Chrome'} · A.I học thuật có kiểm soát nguồn</small></span></div>
