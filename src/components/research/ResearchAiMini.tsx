...
export default function ResearchAiMini({member,works,query,onOpenProposal}:Props){
  ...
  const ask=async()=>{const text=clean(input||query);if(!text||busy)return;const controller=new AbortController();pending.current=controller;setBusy(true);setNote('');try{
    const [drive,liveOpenAlex,central]=await Promise.all([searchDriveRag(text,4,controller.signal),searchOpenAlex(text,6).catch(()=>[]),searchKnowledge(text,'all',5).catch(()=>[])]),evidence=dedupe([...liveOpenAlex,...works]).slice(0,10),literature=asSources(evidence),knowledge=centralSources(central),sources=[...knowledge,...drive.sources,...literature].slice(0,6);
    if(controller.signal.aborted)return;
    if(sources.length){const result=await askServerAi(text,'research',sources,controller.signal);if(controller.signal.aborted)return;setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));const provenance=[central.length?`Central RAG ${central.length}`:'',drive.sources.length?`Drive ${drive.sources.length}`:'',liveOpenAlex.length?`OpenAlex ${liveOpenAlex.length}`:'',result.provider].filter(Boolean).join(' · ');setNote(result.degraded?`Nguồn đã truy xuất: ${provenance}. Cloud A.I đang chuyển chế độ nên dùng fallback học thuật có nguồn.`:`Nguồn: ${provenance} · ${Math.round(result.latencyMs)} ms`)}
    else{const result=await askServerAi(text,'research',[],controller.signal);if(controller.signal.aborted)return;setAnswer(result.degraded?buildAcademicFallback(text,evidence):renderAiAnswer(result));setNote(result.degraded?'Drive/Central RAG/OpenAlex chưa có nguồn phù hợp; dùng fallback local.':'A.I cloud chưa có nguồn truy xuất trong lượt này; cần kiểm chứng.')}
