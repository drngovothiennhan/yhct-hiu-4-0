...
export default function ResearchAiMini({member,works,query,onOpenProposal}:Props){
 ...
 const ask=async(raw?:string)=>{const text=clean(raw||input||query);if(!text||busy)return;const controller=new AbortController();pending.current=controller;setBusy(true);setNote('Gemini Leader đang phân công nguồn…');setMessages(xs=>[...xs,threadMessage('user',text)].slice(-18));setInput('');try{
   ...
   setMessages(xs=>[...xs,threadMessage('assistant',answer,{citations,meta:`Gemini Leader · ${workers.join(' · ')}${result.degraded?' · fallback':''}`})].slice(-18));
