...
export default function AcademicFeed({posts,setPosts,member,reload,composeNonce=0}:{posts:AcademicPost[];setPosts:React.Dispatch<React.SetStateAction<AcademicPost[]>>;member:Member|null;reload:()=>Promise<void>;composeNonce?:number}){
  ...
  const shown=useMemo(()=>posts.filter(p=>(!isSystemAiPost(p)||visibleSystemAiIds.has(p.id))&&(!tag||p.tags.includes(tag))&&(!q||`${p.title} ${p.syndrome} ${p.tags.join(' ')}`.toLowerCase().includes(q.toLowerCase()))),[posts,q,tag,visibleSystemAiIds]);
  ...
  return <section className={`feed-layout${desktopWidgets?' has-desktop-widgets':''}`} data-system-ai-visible={visibleSystemAiIds.size}>
    ...
    <div className="news-rail" aria-label="Điểm tin Y học cổ truyền tích hợp trong bảng tin"><TcmNewsRotator/></div><CommunitySidebar/>
  </section>;
}
