...
export default function AcademicPostCard({post,member,onBookmark,onFollow,onLike,onComment,onRepost,onEdit,onDelete}:{post:AcademicPost;member:Member|null;onBookmark:(p:AcademicPost)=>void;onFollow:(p:AcademicPost)=>void;onLike:(p:AcademicPost)=>void;onComment:(p:AcademicPost)=>void;onRepost:(p:AcademicPost)=>void;onEdit:(p:AcademicPost)=>void;onDelete:(p:AcademicPost)=>void}){
  ...
  return <article className="post-card">
    ...
    <h3>{post.title}</h3>
    <p className="post-lead"><b>Chủ chứng:</b> {post.chiefComplaint}</p>
