...
export default function QuizImportCenter(){
 ...
 useEffect(()=>{
  ...
  if(admin){
   void quizWorkspace<{roots:QuizDriveItem[];driveConfigured?:boolean;credentialMode?:string}>('quiz-roots').then(x=>{
    ...
    if(x.driveConfigured===false)setMessage('Google Drive chưa có credential server-side. Bạn vẫn có thể tải DOCX/TXT/PDF trực tiếp và nhập tên chủ đề bên dưới.');
 ...
 return <section className="panel quiz-import" aria-label="Tài liệu thành ngân hàng trắc nghiệm">
  <div className="qi-heading"><div><span className="qi-kicker"><Sparkles/>ACC · Gemini Quiz Designer</span><h2>Tài liệu → Ngân hàng trắc nghiệm</h2><p>Chọn thư mục Drive hoặc tải DOCX/TXT/PDF trực tiếp tại ACC. Hệ thống nhận diện câu có sẵn hoặc giao Gemini thiết kế câu hỏi chỉ từ nội dung nguồn. Mọi câu chỉ được nhập sau khi admin mở bản nháp, đối chiếu và chủ động xác nhận.</p></div><Bot/></div>
  ...
  {driveConfigured===false&&<div className="qi-drive-warning" role="alert"><b>Drive tạm chưa khả dụng</b><span>Thiếu credential Google Drive phía server. Chức năng tải DOCX/TXT/PDF trực tiếp vẫn hoạt động và không cần Drive.</span></div>}
