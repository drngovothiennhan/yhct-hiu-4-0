...
export default function AiOperationsPanel(){
  ...
  return <section className="panel admin-v6-block ai-operations-panel" data-ai-ops-surface="acc">
    ...
    <div className="admin-v6-grid">
      <article className="admin-v6-block"><div className="row"><Cloud/><h4>Runtime & Provider Router</h4></div><p className="muted">A.I Mini, Research A.I và Exam Tutor đi qua Gateway chung. Research/Exam có thể failover sang Gemini server-side; luồng có Drive riêng tư bị chặn khỏi Gemini theo mặc định.</p><div className="keys"><span className="badge">Structured output {ai?.structuredOutputs?'✓':'—'}</span><span className="badge">Function calling {ai?.functionCalling?'✓':'—'}</span><span className="badge">RBAC {ai?.roleBoundTools?'✓':'—'}</span><span className="badge">Private→Gemini {ai?.privateContextToGemini?'ON':'OFF'}</span></div></article>
