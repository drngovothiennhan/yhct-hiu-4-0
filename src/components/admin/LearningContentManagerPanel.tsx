...
export default function LearningContentManagerPanel(){
  ...
  return <section className="panel quiz-import publish-center-v2 quiz-bank-manager" aria-label="Ngân hàng đề thi">
    ...
    <div className="quiz-bank-update-card quiz-bank-canonical">
      <div className="quiz-bank-update-main"><FolderOpen/><div><b>Thêm thủ công → Cập nhật → dùng ngay</b><small>Tên thư mục môn là nội dung người học nhìn thấy. Hệ thống quét toàn bộ cây thư mục; tài liệu trong thư mục con vẫn về đúng chủ đề cấp 1. DOCX và Google Docs được chuẩn hóa về DOCX nội bộ trước khi nhận diện.</small></div><button type="button" className="qi-primary-convert quiz-bank-update-button" disabled={busy} onClick={()=>void update()}><RefreshCcw className={busy?'spin':''}/>{busy?'Đang cập nhật…':'Cập nhật lại'}</button></div>
