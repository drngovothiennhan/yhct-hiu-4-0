...
const subjectFromFileName=(name:string)=>name.replace(/\.(?:docx|txt|pdf)$/i,'').replace(/^\s*\d+[._ -]*/,'').replace(/[_-]+/g,' ').trim()||'Chưa phân loại';
...
export default function LearningContentManagerPanel(){
  ...
  const updateQuizBank=async()=>{
    ...
    setMessage(`Cập nhật xong · ${processed.length} tệp đã kiểm tra · ${imported} câu cập nhật${waiting?` · ${waiting} tệp chưa đủ điều kiện`:''}${errors?` · ${errors} tệp lỗi`:''}${remaining?` · còn ${remaining} tệp mới`:''}${localWaiting?` · ${localWaiting}`:''}.`);
  };
