...
export default function UnifiedAiMini({member,onLogin}:{member:Member|null;onLogin:()=>void}){
  ...
  const runQuery=async(raw:string)=>{const text=clean(raw);if(!text||busy)return;if(!member){onLogin();return}setBusy(true);setMessage('');recordAiUse(member.id);const isAcademic=academicIntent(text),contextBase=moduleContext||`${location.pathname}${location.search}`,continuity=buildConversationContext(isAcademic?messages.filter(item=>item.academic):messages,contextBase);const userMessage:Message={id:crypto.randomUUID(),role:'user',text,academic:isAcademic};setMessages(xs=>[...xs,userMessage].slice(-10));setQuery('');try{
    if(appIntent(text)){addAssistant(appHelp(text));return}
    if(isAcademic){const reply=await askAcademicUnified(text,[],continuity);addAssistant(reply.answer,reply.sources,reply.suggestedQueries,true);setMessage(`Học thuật có nguồn · ${reply.provenance||reply.provider}${reply.degraded?' · fallback':''}${reply.latencyMs?` · ${Math.round(reply.latencyMs)} ms`:''}`);return}
