...
function displayTitle(item:CommunityEntry){if(item.group_key==='leadership')return item.rank_no===1?'Chủ nhiệm':'Phó Chủ nhiệm';if(item.group_key==='management')return'Ban quản lý';return item.credit_rank||'Chưa xếp hạng'}
...
function Block({title,subtitle,items,icon,showRank=false}:{title:string;subtitle:string;items:CommunityEntry[];icon:ReactNode;showRank?:boolean}){
  return <section className="community-block"><header><div className="row">{icon}<div><h3>{title}</h3><small>{subtitle}</small></div></div></header><div className="community-member-list">{items.length?items.map(item=><MemberRow key={`${item.group_key}-${item.member_id}`} item={item} showRank={showRank}/>):<div className="community-empty">Chưa có dữ liệu.</div>}</div></section>;
}

export default function CommunitySidebar(){
  const [rows,setRows]=useState<CommunityEntry[]>([]),[msg,setMsg]=useState(''),[managementPage,setManagementPage]=useState(0),[activePage,setActivePage]=useState(0);
  useEffect(()=>{
    let mounted=true;
    const load=async()=>{try{const {data,error}=await supabase.rpc('community_sidebar_v2');if(error)throw error;if(mounted){setRows((data||[]) as CommunityEntry[]);setMsg('')}}catch(error){if(mounted)setMsg(error instanceof Error?error.message:'Không thể tải thông tin cộng đồng.')}};
    void load();const refreshId=window.setInterval(()=>void load(),REFRESH_MS);return()=>{mounted=false;window.clearInterval(refreshId)};
  },[]);
  ...
  return <aside className="community-sidebar" aria-label="Thông tin cộng đồng YHCT HIU">
    ...
    <Block title="Ban chủ nhiệm" subtitle="Cơ cấu điều hành · tín dụng chỉ là hoạt động cộng đồng" items={leadership} icon={<ShieldCheck/>}/>
    <Block title="Ban quản lý" subtitle="Tự động chuyển · 3 thành viên/lượt" items={visiblePage(management,managementPage)} icon={<Users/>}/>
    <Block title="Thành viên tích cực" subtitle="Top 10 Tín dụng Cộng đồng · cộng dồn trọn đời" items={visiblePage(active,activePage)} icon={<Award/>} showRank/>
  </aside>;
}
