...
export default function NotificationsCenter({member}:{member:Member}){
  ...
  const load=async()=>{setBusy(true);setMsg('');try{const {data,error}=await supabase.from('notifications').select('id,kind,title,body,post_id,read_at,created_at').eq('member_id',member.id).order('created_at',{ascending:false}).limit(50);if(error)throw error;setItems((data||[]) as NotificationRow[])}catch(e){setMsg((e as Error).message||'Không thể tải thông báo')}finally{setBusy(false)}};
