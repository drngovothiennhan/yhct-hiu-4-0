...
export default function PracticeBankQuiz(){
  ...
  const chooseSource=(next:QuizSource)=>{if(busy||questions.length)return;setSourceMode(next);setMessage('');setAiSources([]);if(next==='ai'){setCount(current=>AI_QUESTION_COUNTS.includes(current as 5|10|20)?current:10);setAiTopic(current=>current||subject)}else setCount(current=>HIU_QUESTION_COUNTS.includes(current as 10|20|30|50)?current:20)};
  ...
  const startAi=async()=>{const topic=aiTopic.replace(/\s+/g,' ').trim();if(topic.length<2)throw new Error('Hãy nhập chủ đề muốn Gemini tạo đề.');setAnswers({});setResult(null);setReviewRequested(new Set());setAiSources([]);const quiz=await generateStudyGeminiQuiz(topic,count);const generation=crypto.randomUUID();setQuestions(quiz.questions.map((question,index)=>({id:`ai:${generation}:${index}`,subject:quiz.topic,topic:'A.I tạo · Google Search',stem:question.stem,options:question.options,sourceFileName:'Gemini · Google Search',generationMethod:'ai_generated',aiGenerated:true,aiCorrectIndex:question.correctIndex,aiExplanation:question.explanation})));setAiSources(quiz.sources);if(quiz.questions.length<count)setMessage(`Gemini đã tạo ${quiz.questions.length} câu hợp lệ có nguồn web cho chủ đề này.`)};
  ...
  return <section className="panel practice-bank" aria-label="Quiz học tập HIU và Gemini">
    <header className="practice-bank__head"><div><span><FileQuestion/> QUIZ HỌC TẬP</span><h2>Chọn nguồn → nội dung → số câu → bắt đầu</h2><p>Chọn ngân hàng đề HIU đã duyệt hoặc để Gemini tham khảo web và tạo đề mới theo chủ đề của bạn.</p></div>{guest&&<b>Đăng nhập để làm quiz</b>}</header>

    {questions.length===0&&<div className="practice-bank__source-switch" role="group" aria-label="Chọn nguồn đề">
      <button type="button" className={sourceMode==='hiu'?'active':''} disabled={guest||busy} onClick={()=>chooseSource('hiu')}><GraduationCap/><span><b>Ngân hàng đề HIU</b><small>Nội dung đã được duyệt</small></span></button>
      <button type="button" className={sourceMode==='ai'?'active ai':''} disabled={guest||busy} onClick={()=>chooseSource('ai')}><Bot/><span><b>Đề do Gemini tạo</b><small>Google Search · gắn nhãn A.I tạo</small></span></button>
    </div>}
    ...
    <div className="practice-bank__controls">
      ...
      {questions.length===0?<button className="primary" disabled={busy||!config?.ready||(sourceMode==='ai'&&aiTopic.trim().length<2)} onClick={()=>void start()}>{busy?<LoaderCircle className="spin"/>:sourceMode==='ai'?<Sparkles/>:<FileQuestion/>}{guest?'Cần đăng nhập':busy&&sourceMode==='ai'?'Gemini đang tìm nguồn...':sourceMode==='ai'?'Gemini tạo đề':'Bắt đầu đề HIU'}</button>:<button className="secondary" disabled={busy} onClick={reset}><RotateCcw/>Chọn lại</button>}
    </div>
    ...
    {sourceMode==='ai'&&questions.length===0&&!message&&<div className="practice-bank__ai-note"><Globe2/><span><b>Đề A.I tạo</b><small>Gemini tìm nguồn web công khai trước khi tạo câu hỏi. Đề tạm thời này không ghi vào ngân hàng HIU.</small></span></div>}
