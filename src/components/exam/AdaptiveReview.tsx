...
export default function AdaptiveReview({identity}:{identity:string|null}){
 ...
 const [now,setNow]=useState(Date.now()),[subject,setSubject]=useState(''),[count,setCount]=useState<number>(10),[reviewed,setReviewed]=useState(0);
 useEffect(()=>{setReviewed(0);setRevealed(false)},[identity,subject,count]);
 ...
 const sessionCards=due.slice(0,count===0?due.length:Math.max(0,count-reviewed)),card=sessionCards[0];
 ...
 {sessionCards.length>0&&<small>Phiên hiện tại tối đa {count===0?due.length:Math.min(Math.max(0,count-reviewed),due.length)} thẻ · còn {due.length} thẻ đến hạn trong bộ lọc.</small>}{notice&&<p role="status">{notice}</p>}</section>;
}