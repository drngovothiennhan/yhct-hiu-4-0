...
export default function DailyDrivePractice({onChooseGemini}:DailyDrivePracticeProps){
  ...
  const openToday=async(count=selectedCount)=>{setBusy(true);setMsg('');try{const next=await getTodayDailyPractice(count);setSession(next);setIdx(0);if(!next.ready)setMsg('Ngân hàng chưa có câu hỏi đã xác minh. Vui lòng quay lại sau khi admin cập nhật.')}catch(e){setMsg((e as Error).message)}finally{answerLock.current=false;setBusy(false)}};
  useEffect(()=>{let alive=true;void getDailyPracticeConfig().then(async next=>{if(!alive)return;setConfig(next);if(next.authenticated&&next.hasTodaySession){const s=await getTodayDailyPractice(next.defaultCount||10);if(alive){setSession(s);const locked=(s.questionCount||10);if(locked===5||locked===10||locked===20)setSelectedCount(locked)}}}).catch(e=>{if(alive)setMsg((e as Error).message)});return()=>{alive=false}},[]);
