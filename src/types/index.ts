...
export type AcademicPostType='research'|'clinical_case'|'medicinal_diet'|'news'|'reference'|'status';
...
export interface Member { id:string; studentCode?:string; fullName:string; email?:string; phone?:string; role:SystemRole; title:AppointmentTitle; status:MemberStatus; reputation:number; totalPoints:number; avatarUrl?:string; herbalAlias?:string; wallTheme?:string; wallMotto?:string; departmentSlug?:string; elementRank?:string; loginEnabled?:boolean; dataConflict?:boolean; }
...
export interface AcademicMedia { id:string; name:string; url:string; aspect:'16:9'|'1:1'; width:number; height:number; }
export interface AcademicPost { id:string; author:Member; title:string; chiefComplaint:string; fourExams:FourExams; eightPrinciples:string[]; syndrome:string; treatmentPrinciple:string; formula?:string; acupoints?:string[]; citations:string[]; tags:string[]; createdAt:string; approvedAt?:string; modVerified:boolean; citationVerified?:boolean; moderationStatus?:'pending'|'approved'|'rejected'|'quarantined'; postType?:AcademicPostType; specialty?:string; visibility?:'public'|'members'; media?:AcademicMedia[]; likes:number; agrees:number; comments:number; bookmarked?:boolean; followingAuthor?:boolean; }
