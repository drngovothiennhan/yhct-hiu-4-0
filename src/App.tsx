...
export default function App(){
  ...
  const login=async()=>{if(loginGuard.current)return;loginGuard.current=true;setBusy(true);setErr('');setAuthPhase('Đang kết nối máy chủ xác thực…');try{const m=await loginOptimized(student.trim(),password);setMember(m);setAuthResolved(true);setAuthPhase('Đăng nhập thành công');setAuthOpen(false);setPassword('');void reload()}catch(e){setPassword('');setErr((e as Error).message||'Không thể đăng nhập');setAuthPhase('');window.requestAnimationFrame(()=>passwordInputRef.current?.focus())}finally{loginGuard.current=false;setBusy(false)}};
  const logout=()=>{setMember(null);setNotificationUnread(null);setTab('feed');setMoreOpen(false);replaceRoute('feed');logoutFast()};
