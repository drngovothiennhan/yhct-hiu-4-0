...
export default function App(){
  ...
  return <div className={`app viewport-mode-${viewportMode}`}>
    <aside>
      ...
      <nav><button className={tab==='feed'?'active':''} onClick={()=>go('feed')}><BookOpen/>Bảng tin học thuật</button><button className={tab==='research'?'active':''} onClick={()=>go('research')}><FlaskConical/>Trung tâm nghiên cứu</button>{member&&<button className={tab==='profile'?'active':''} onClick={()=>go('profile')}><Medal/>Hồ sơ học thuật</button>}{member&&<button className={tab==='notifications'?'active':''} onClick={()=>go('notifications')}><Bell/>Thông báo</button>}<button className={tab==='schedule'?'active':''} onClick={()=>go('schedule')}><CalendarDays/>Lịch hoạt động</button><button className={tab==='drl'?'active':''} onClick={()=>go('drl')}><FiveElementsIcon/>Điểm hoạt động</button><button className={tab==='exam'?'active':''} onClick={()=>go('exam')}><GraduationCap/>Luyện thi ĐGNL</button>{canAdmin&&<button className={tab==='admin'?'active':''} onClick={()=>go('admin')}><ShieldCheck/>Điều hành</button>}{canAcc&&<button className={`acc-nav ${tab==='acc'?'active':''}`} onClick={()=>go('acc')}><Star/>ACC Hệ thống</button>}</nav>
