...
import { Bell,BookOpen,CalendarDays,Compass,FlaskConical,GraduationCap,Home,LogIn,LogOut,Medal,Menu,Plus,ShieldCheck,Sparkles,Star,UserRound,X } from 'lucide-react';
...
export default function App(){
  ...
  const loginGuard=useRef(false),lastClassicThemeRef=useRef<ThemeName>(theme==='tcm-cartoon-2d'?'duoc-ngoc':theme),canAdmin=roleAtLeast(member?.role,'mod'),canAcc=roleAtLeast(member?.role,'admin');
  ...
  const toggleCartoonTheme=()=>{if(theme==='tcm-cartoon-2d'){setTheme(lastClassicThemeRef.current==='tcm-cartoon-2d'?'duoc-ngoc':lastClassicThemeRef.current);return}lastClassicThemeRef.current=theme;setTheme('tcm-cartoon-2d')};
  ...
  return <div className={`app viewport-mode-${viewportMode}`}>
    <aside>
      ...
      <nav><button className={tab==='feed'?'active':''} onClick={()=>setTab('feed')}><BookOpen/>Bảng tin học thuật</button><button className={tab==='research'?'active':''} onClick={()=>setTab('research')}><FlaskConical/>Trung tâm nghiên cứu</button>{member&&<button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}><Medal/>Hồ sơ học thuật</button>}{member&&<button className={tab==='notifications'?'active':''} onClick={()=>setTab('notifications')}><Bell/>Thông báo</button>}<button className={tab==='schedule'?'active':''} onClick={()=>setTab('schedule')}><CalendarDays/>Lịch hoạt động</button><button className={tab==='drl'?'active':''} onClick={()=>setTab('drl')}><FiveElementsIcon/>Điểm hoạt động</button><button className={tab==='exam'?'active':''} onClick={()=>setTab('exam')}><GraduationCap/>Luyện thi ĐGNL</button>{canAdmin&&<button className={tab==='admin'?'active':''} onClick={()=>setTab('admin')}><ShieldCheck/>Điều hành</button>}{canAcc&&<button className={`acc-nav ${tab==='acc'?'active':''}`} onClick={()=>setTab('acc')}><Star/>ACC Hệ thống</button>}</nav>
