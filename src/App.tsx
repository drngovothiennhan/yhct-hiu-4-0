...
import UnifiedAiMini from './components/ai/UnifiedAiMini';
...
const TITLES:Record<Tab,string>={feed:'Bảng tin học thuật',research:'Khám phá học thuật',profile:'Hồ sơ & Tường cá nhân',schedule:'Lịch hoạt động',exam:'Luyện thi Đánh giá Năng lực',drl:'Điểm hoạt động / Rèn luyện',notifications:'Thông báo',garden:'Gia Viên Dược Thảo',messages:'Inbox cá nhân',admin:'Điều hành thành viên',acc:'Admin Control Center'};
...
function normalizePath(pathname:string){const value=pathname.replace(/\/+$/,'')||'/';return value.startsWith('/')?value:`/${value}`}
function tabFromLocation():Tab{if(typeof window==='undefined')return'feed';if(window.location.pathname==='/profile')return'profile';return PATH_TABS.get(normalizePath(window.location.pathname))||'feed'}
...
export default function App(){
  ...
  return <div className={`app viewport-mode-${viewportMode}`}>
    <aside><div className="brand"><div className="brand-logo brand-logo--system"><SystemBrandMark variant="taiji" label="Biểu trưng hệ thống YHCT HIU 4.0"/></div><div><b>{BRANDING.platformName}</b><small>{BRANDING.owner} · {BRANDING.faculty}</small></div></div>
      <nav><button className={tab==='feed'?'active':''} onClick={()=>go('feed')}><BookOpen/>Bảng tin học thuật</button><button className={tab==='research'?'active':''} onClick={()=>go('research')}><FlaskConical/>Trung tâm nghiên cứu</button>{member&&<><button className={tab==='messages'?'active':''} onClick={()=>go('messages')}><MessageCircle/>Inbox cá nhân</button><button className={tab==='garden'?'active':''} onClick={()=>go('garden')}><Gamepad2/>Gia Viên Dược Thảo</button><button className={tab==='profile'?'active':''} onClick={()=>go('profile')}><Medal/>Tường cá nhân</button><button className={tab==='notifications'?'active':''} onClick={()=>go('notifications')}><Bell/>Thông báo</button></>}<button className={tab==='schedule'?'active':''} onClick={()=>go('schedule')}><CalendarDays/>Lịch hoạt động</button><button className={tab==='drl'?'active':''} onClick={()=>go('drl')}><FiveElementsIcon/>Điểm hoạt động</button><button className={tab==='exam'?'active':''} onClick={()=>go('exam')}><GraduationCap/>Luyện thi ĐGNL</button>{canAdmin&&<button className={tab==='admin'?'active':''} onClick={()=>go('admin')}><ShieldCheck/>Điều hành</button>}{canAcc&&<button className={`acc-nav ${tab==='acc'?'active':''}`} onClick={()=>go('acc')}><Star/>ACC Hệ thống</button>}</nav>
      <footer>{member?<><b>{member.herbalAlias||member.fullName}</b><small>{member.fullName} · {member.title}</small><button onClick={logout}><LogOut/>Đăng xuất</button></>:<button onClick={()=>setAuthOpen(true)}><LogIn/>Đăng nhập thành viên</button>}<ViewportModeToggle mode={viewportMode} onChange={changeViewport} className="viewport-toggle--sidebar"/></footer>
    </aside>
    <main><header className="top"><TcmCartoonDecor/><div className="top-title"><SystemBrandMark variant="taiji" size={40} className="mobile-top-mark" label="YHCT HIU 4.0"/><div><h1>{TITLES[tab]}</h1><p>{BRANDING.subtitle}</p></div></div><div className="top-actions"><span className="badge">FINAL 4.0 · RESPONSIVE</span><button className="mobile-more-button" onClick={()=>setMoreOpen(true)} aria-label="Mở thêm chức năng" title="Thêm"><Menu/></button><button className="mobile-account-button" onClick={member?logout:()=>setAuthOpen(true)} aria-label={member?'Đăng xuất':'Đăng nhập thành viên'} title={member?'Đăng xuất':'Đăng nhập thành viên'}>{member?<LogOut/>:<LogIn/>}</button></div></header>
      ...
      {tab==='messages'&&member&&<Suspense fallback={<LazyFallback/>}><MessagesCenter member={member}/></Suspense>}{tab==='garden'&&member&&<Suspense fallback={<LazyFallback/>}><HerbGardenGame member={member}/><HerbGardenSocialHub member={member}/></Suspense>}
