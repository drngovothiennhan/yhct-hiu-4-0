...
const StudentLibrary=lazy(()=>import('./components/library/StudentLibrary'));
...
export default function App(){
  ...
  const loginGuard=useRef(false),notificationMemberRef=useRef<string|null>(member?.id||null),canAdmin=roleAtLeast(member?.role,'mod'),canAcc=roleAtLeast(member?.role,'admin'),canLearning=canManageLearningContent(member),learningOnly=canLearning&&!canAdmin;
  ...
  const openAssistant=()=>requestMemberAction(()=>window.dispatchEvent(new CustomEvent('yhct:ai:open',{detail:{context:'phase15-study-shell'}})));
  ...
  const moduleContent=<ModuleBoundary moduleId={tab}><Suspense fallback={<LazyFallback/>}>
    ...
    {tab==='library'&&<StudentLibrary member={member} onLogin={()=>setAuthOpen(true)} onNavigate={go}/>} 
