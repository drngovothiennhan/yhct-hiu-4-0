...
export default function App(){
  ...
  useEffect(()=>{if(!authResolved)return;const definition=MODULES[tab];let fallback:ModuleId|null=null;if(definition.memberOnly&&!member)fallback='feed';else if(definition.minRole&&!roleAtLeast(member?.role,definition.minRole))fallback='feed';if(fallback){setTab(fallback);replaceRoute(fallback)}},[authResolved,tab,member]);
