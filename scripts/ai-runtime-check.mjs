...
const researchEvidenceClient=read('src/services/researchEvidenceService.ts');
...
requireText(research,'searchResearchEvidence(query,18)','Research Center uses one canonical public evidence retrieval');
...
requireText(researchEvidenceClient,"/api/knowledge/resources?action=research",'Research evidence client uses existing knowledge serverless boundary');
requireText(researchResourceApi,"action==='research'",'knowledge serverless function owns Research retrieval action');
requireText(researchResourceApi,'retrievePublicResearchEvidence','Research server action uses centralized medical evidence retriever');
...
requireText(researchMini,'searchResearchEvidence(text,14,controller.signal)','Research A.I uses canonical server evidence retrieval');
requireText(researchMini,'reusePublic','Research A.I reuses matching public results before searching again');
for(const old of ['searchPubMed(text,8)','searchOpenAlex(text,8)','searchClinicalTrials(text,5)'])forbidText(researchMini,old,`Research A.I excludes duplicate provider fan-out ${old}`);
