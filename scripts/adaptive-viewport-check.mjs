...
async function runCase(c,index){
  ...
  try{
    ...
    if(c.forceDesktop){
      ...
      for(const child of s.mainChildren||[]){const x=child.rect;if(x&&(x.left<s.main.left-2||x.right>s.main.right+2))throw new Error(`${c.name}: top-level main child escapes compact desktop column ${JSON.stringify(child)} ${JSON.stringify(s)}`)}
