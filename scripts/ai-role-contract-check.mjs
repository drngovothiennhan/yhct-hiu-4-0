...
need(quizManager,['Ngân hàng câu hỏi','syncQuizBank','updateQuizBank','Cập nhật','File từ ACC','DOCX · PDF · TXT','tryTrustedQuizUpload','processUpload','sourceFileBase64',"conversionMode:'auto'",'Xử lý nâng cao / tài liệu ngoại lệ','không được dùng A.I suy đoán','publishAll','Duyệt & phát hành','driveReady','subject.trim()','AnswerReviewQueue'],'canonical universal ACC quiz-bank manager with one-step trusted Update and explicit exception review');
forbid(quizManager,['selection:ready','setChecked(new Set(d.questions'],'canonical ACC explicit admin review');
need(trustedIngest,["BANK_FOLDER_NAME='NGÂN HÀNG TRẮC NGHIỆM'","MANUAL_INTAKE_FOLDER='Thêm thủ công'",'practice_trusted_quiz_ingest_v1','practice_source_sync_state_v1','parseTrustedMarkedDocx','parseTrustedDeterministicSource','isSupportedSource','PDFParse','explicit-answer-key-v1'],'trusted universal one-step Drive/ACC bank update');
...
console.log('AI role contract PASS: one task assistant, one Gemini Study role, one Research A.I role, one canonical universal ACC/Drive quiz-bank Update, request-scoped internal consent, deterministic trusted answers, provenance/admin exception review, truthful configured-vs-live observability and anti-sprawl boundaries are enforced.');
