...
const required=[
  ...
  'scripts/role-ui-audit.mjs','public/service-worker.js','public/yhct-system-mark.svg','ops/sql/community_sidebar_v1.sql'
];
const errors=[];
...
for(const token of ['localStorage','onPointerDown','visualViewport','Notification.requestPermission','Điểm kỳ này của tôi','Gemini BYOK','Tra cứu trực tiếp','Kho YHCT','Google Drive'])if(!widget.includes(token))errors.push(`Mini AI gate missing ${token}`);
...
for(const token of ['PAGE_SIZE=2','ROTATE_MS=8000','REFRESH_MS=5*60*1000','tcm_news_feed_v1','visibleItems','target="_blank"','news-rotator-text','Đọc nguồn'])if(!rotator.includes(token))errors.push(`news rotator gate missing ${token}`);
for(const token of ['touchmove','touchstart','scrollLeft','scrollBy(','scrollTo(','onScroll=','setPointerCapture','releasePointerCapture','dragRef'])if(rotator.includes(token))errors.push(`news rotator must not depend on horizontal gestures: ${token}`);
const rotatorCss=read('src/news-rotator.css');
for(const token of ['.news-rotator-pair','-webkit-line-clamp:3','.news-rotator-text'])if(!rotatorCss.includes(token))errors.push(`news rotator CSS gate missing ${token}`);
...
const sql=read('ops/sql/community_sidebar_v1.sql');
for(const token of ['community_sidebar_v1','status=\'approved\'','role in (\'admin\',\'super_mod\')',"role='mod'","role='member'",'limit 10','sum(st.points)'])if(!sql.includes(token))errors.push(`community sidebar SQL gate missing ${token}`);
...
for(const token of ['drl_admin_import_v1','drl_admin_upsert_semester_v1','Đã nhận mẫu đề nghị ĐRL HIU','matchDetectedSemester','duplicateStudentCodes','structuredImport'])if(!drl.includes(token))errors.push(`DRL structured import gate missing ${token}`);
...
if(!sw.includes('ui11-desktop-community-sidebar'))errors.push('service worker cache version did not advance for desktop community release');

if(errors.length){console.error('ACCEPTANCE CHECK FAILED');errors.forEach(error=>console.error(`- ${error}`));process.exit(1)}
console.log(`acceptance-ok: ${files.length} application source files scanned; mobile layout locked; desktop news centered at top; 3-block community sidebar + top-10 activity ranking + 4-role/RBAC/DRL/security gates clean`);
