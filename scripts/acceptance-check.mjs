...
const required=[
 ...
 'src/components/system/ViewportModeToggle.tsx','src/academic-production.css','src/social-v5.css','src/news-rotator.css','src/desktop-community.css','src/desktop-interaction-profile.css','src/final4-v2.css','public/service-worker.js','public/manifest.webmanifest','vite.config.ts','vercel.json','scripts/role-ui-audit.mjs'
];
...
console.log(`acceptance-ok: ${sourceFiles.length} application source files scanned; social v5 + two-line references + frozen mobile + inbox + wall + moderation + garden economy + DRL + PWA gates clean`);
