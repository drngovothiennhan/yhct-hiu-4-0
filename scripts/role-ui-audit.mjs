...
const files={app:read('src/App.tsx'),boot:read('index.html'),viewport:read('src/components/system/ViewportModeToggle.tsx'),css:read('src/viewport-native-hotfix.css')+read('src/news-rotator.css')+read('src/final4-v2.css')+read('src/social-v5.css')+read('src/garden-v6.css'),desktop:read('src/desktop-community.css')+read('src/desktop-interaction-profile.css'),news:read('src/components/news/TcmNewsRotator.tsx'),community:read('src/components/community/CommunitySidebar.tsx'),admin:read('src/components/admin/SystemAdminCenter.tsx'),adminTheme:read('src/components/admin/AdminThemeControl.tsx'),moderation:read('src/components/admin/ModerationOpsPanel.tsx'),drl:read('src/components/drl/DrlCenter.tsx'),worker:read('src/workers/drlParseWorker.ts'),feed:read('src/components/feed/AcademicFeed.tsx'),post:read('src/components/feed/AcademicPostCard.tsx'),mini:read('src/components/ai/UnifiedAiMini.tsx'),driveApi:read('api/research/drive.js'),docx:read('src/components/feed/DocxImportPanel.tsx'),types:read('src/types/index.ts'),social:read('src/services/socialService.ts'),theme:read('src/theme.ts'),profile:read('src/components/profile/ProfileCenter.tsx'),auth:read('src/services/authService.ts'),authRuntime:read('src/services/authRuntimeService.ts'),garden:read('src/components/game/HerbGardenGame.tsx'),gardenSocial:read('src/components/game/HerbGardenSocialHub.tsx'),messages:read('src/components/messages/MessagesCenter.tsx'),composer:read('src/components/feed/AcademicPostComposer.tsx'),vite:read('vite.config.ts'),main:read('src/main.tsx')};
...
let failed=0;for(const [role,items] of checks){const bad=items.filter(([,ok])=>!ok);if(bad.length){failed++;console.error(`role-audit ${role} FAIL: ${bad.map(([name])=>name).join('; ')}`)}else console.log(`role-audit ${role} PASS: ${items.map(([name])=>name).join('; ')}`)}if(failed)process.exit(1);console.log('role-audit-ok: social v6, frozen mobile, portable hosting, private inbox, wall identity, moderation queues, nine-plot garden economy, DRL and RBAC contracts passed');
