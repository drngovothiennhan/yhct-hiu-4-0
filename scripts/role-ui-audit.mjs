...
const files={
 ...
 css:read('src/viewport-native-hotfix.css')+read('src/news-rotator.css')+read('src/final4-v2.css')+read('src/social-v5.css')+read('src/garden-v6.css')+read('src/module-isolation.css')+read('src/xiaozhi-mini.css')+read('src/app-assistant-ai.css'),
 news:read('src/components/news/TcmNewsRotator.tsx'),community:read('src/components/community/CommunitySidebar.tsx'),admin:read('src/components/admin/SystemAdminCenter.tsx'),adminControl:read('src/components/admin/AdminControlCenter.tsx'),learning:read('src/components/admin/LearningContentManagerPanel.tsx'),adminTheme:read('src/components/admin/AdminThemeControl.tsx'),moderation:read('src/components/admin/ModerationOpsPanel.tsx'),drl:read('src/components/drl/DrlCenter.tsx'),worker:read('src/workers/drlParseWorker.ts'),feed:read('src/components/feed/AcademicFeed.tsx'),post:read('src/components/feed/AcademicPostCard.tsx'),mini:read('src/components/ai/UnifiedAiMini.tsx'),miniService:read('src/services/xiaozhiMiniService.ts'),aiCenter:read('src/components/ai/AiCenter.tsx'),studyService:read('src/services/studyAiService.ts'),docx:read('src/components/feed/DocxImportPanel.tsx'),types:read('src/types/index.ts'),theme:read('src/theme.ts'),profile:read('src/components/profile/ProfileCenter.tsx'),profileInbox:read('src/components/profile/ProfileInbox.tsx'),auth:read('src/services/authService.ts'),authRuntime:read('src/services/authRuntimeService.ts'),garden:read('src/components/game/HerbGardenGame.tsx'),gardenSocial:read('src/components/game/HerbGardenSocialHub.tsx'),messages:read('src/components/messages/MessagesCenter.tsx'),composer:read('src/components/feed/AcademicPostComposer.tsx'),vite:read('vite.config.ts'),main:read('src/main.tsx'),contract:read('src/modules/moduleContract.ts'),pwa:read('src/services/pwaInstallService.ts'),settings:read('src/components/system/AppSettingsDialog.tsx'),research:read('src/components/research/ResearchCenter.tsx'),researchMini:read('src/components/research/ResearchAiMini.tsx')
};
...
const checks=[
 ['ADMIN',[
  ...
  ['learning manager appointment stays scoped',has(files.types,"LEARNING_CONTENT_MANAGER_TITLE='Ban Quản lý Học tập'","member.role==='admin'||member.title===LEARNING_CONTENT_MANAGER_TITLE")&&has(files.adminControl,"title===LEARNING_CONTENT_MANAGER_TITLE&&picked.role!=='admin'?'member':picked.role",'chỉ quản lý nội dung Học tập','ACC hệ thống không được mở')],
  ['learning manager gets learning-only /admin surface',has(files.app,'canLearning=canManageLearningContent(member)','learningOnly=canLearning&&!canAdmin',"tab==='admin'&&learningOnly&&<LearningContentManagerPanel", "tab==='acc'&&canAcc&&<>")&&has(files.learning,'Ban Quản lý Học tập','quiz-start','quiz-process-chunk','quiz-retry','quiz-commit','practice_question_review_v1')&&not(files.learning,'AdminControlCenter','AdminThemeControl','AdminOpsAssistant','SystemAdminCenter','admin_appoint_member_v1')],
 ...
 ['MOD',[
  ...
  ['Task assistant and Gemini Study are both available',has(files.app,'UnifiedAiMini member={member}')&&has(files.mini,'openStudyAi(text)','VOICE_KEY','startListening')&&not(files.mini,'askXiaoZhiMini')&&has(files.aiCenter,'askStudyGemini')&&has(files.studyService,"fetch('/api/ai/study-assistant'")]
 ]],
 ['USER',[
  ...
  ['Task assistant has no learning-answer or feedback overlap',has(files.mini,'Trợ lý tác vụ','researchIntent','openResearch(text)','openStudyAi(text)','VOICE_KEY','startListening')&&has(files.aiCenter,'AI STUDY OS · GEMINI','askStudyGemini')&&has(files.studyService,"fetch('/api/ai/study-assistant'")&&not(files.mini,'askXiaoZhiMini','askAcademicUnified','feedback_submit_v1','searchOpenAlex','searchPubMed','searchClinicalTrials','searchDriveRag','searchKnowledge')&&not(files.app,'AiMiniFeedbackDock','PersonalCopilotWidget')],
...
console.log('role-audit-ok: modular roles, scoped learning management, atomic theme, rolling TCM news carousel, latest-three AI academic posts, private inbox, Gemini Study + task assistant split, PWA, wall identity, moderation, garden, DRL and RBAC contracts passed');