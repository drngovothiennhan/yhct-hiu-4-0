...
const files={
  ...
  css:read('src/viewport-native-hotfix.css')+read('src/news-rotator.css')+read('src/final4-v2.css'),desktop:read('src/desktop-community.css'),
  ...
  docx:read('src/components/feed/DocxImportPanel.tsx'),types:read('src/types/index.ts'),social:read('src/services/socialService.ts'),theme:read('src/theme.ts')
};
...
const checks=[
  ['ADMIN',[
    ...
    ['leadership fixed block',has(files.community,'Ban chủ nhiệm')&&not(files.community,'leadershipPage')],
  ...
  ['USER',[
    ...
    ['zero-reload 2D YHCT theme',has(files.theme,"'tcm-cartoon-2d'",'localStorage.setItem(KEY,theme)')&&has(files.app,'toggleCartoonTheme','TcmCartoonDecor')]
  ]]
];
let failed=0;
...
console.log('role-audit-ok: Final 4 ADMIN/SUPER_MOD/MOD/USER contracts passed with admin-only DRL publication, PC news interaction, lifetime credits and zero-reload 2D theme');
