...
const checks=[
 ['ADMIN',[
  ...
  ['learning manager gets one-step learning-only /admin surface',has(files.app,'canLearning=canManageLearningContent(member)','learningOnly=canLearning&&!canAdmin',"tab==='admin'&&learningOnly&&<LearningContentManagerPanel", "tab==='acc'&&canAcc&&<>")&&has(files.learning,'Ngân hàng đề thi','syncQuizBank','Thêm thủ công → Cập nhật → dùng ngay','đáp án tô đỏ')&&not(files.learning,'AdminControlCenter','AdminThemeControl','AdminOpsAssistant','SystemAdminCenter','admin_appoint_member_v1','quiz-process-chunk','quiz-commit')],
 ...
 ['USER',[
  ...
  ['Research retrieval remains research-only',has(files.research,'searchOpenAlex(query,12)','<ResearchAiMini')&&has(files.researchMini,'RESEARCH_ROLE=GEMINI_MEDICAL_RESEARCH_LEAD','searchPubMed(text,8)','searchOpenAlex(text,8)','searchClinicalTrials(text,5)','Dùng tài liệu nội bộ cho lượt này')&&has(files.mini,'researchIntent')&&not(files.mini,'searchOpenAlex','searchPubMed','searchClinicalTrials','searchDriveRag','searchKnowledge')],
