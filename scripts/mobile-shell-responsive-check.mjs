...
try{
  ...
  const result=await send('Runtime.evaluate',{expression:`(()=>{const rect=element=>{const value=element?.getBoundingClientRect();return value?{left:value.left,right:value.right,top:value.top,bottom:value.bottom,width:value.width,height:value.height}:null};const visible=element=>Boolean(element&&getComputedStyle(element).display!=='none'&&getComputedStyle(element).visibility!=='hidden');const intersects=(a,b)=>Boolean(a&&b&&a.left<b.right&&a.right>b.left&&a.top<b.bottom&&a.bottom>b.top);const contains=(outer,inner,pad=3)=>Boolean(outer&&inner&&inner.left>=outer.left-pad&&inner.right<=outer.right+pad&&inner.top>=outer.top-pad&&inner.bottom<=outer.bottom+pad);const html=document.documentElement,main=document.querySelector('main'),top=document.querySelector('.top'),title=document.querySelector('.top-title'),content=document.querySelector('.module-boundary'),nav=document.querySelector('.mobile-bottom-nav'),more=document.querySelector('.mobile-more-button'),account=document.querySelector('.mobile-account-button'),slot=document.querySelector('.mobile-assistant-slot'),fab=document.querySelector('.mobile-assistant-slot .xz-orb'),label=document.querySelector('.mobile-assistant-slot .xz-orb>b'),buttons=[...document.querySelectorAll('.mobile-bottom-nav>button')],topRect=rect(top),titleRect=rect(title),contentRect=rect(content),slotRect=rect(slot),fabRect=rect(fab),labelRect=rect(label);return{mode:html.dataset.viewportMode||'',mobileUi:html.dataset.mobileUi||'',innerWidth,docScrollWidth:html.scrollWidth,bodyScrollWidth:document.body.scrollWidth,mainPaddingBottom:main?parseFloat(getComputedStyle(main).paddingBottom)||0:0,main:rect(main),top:topRect,topVisible:visible(top),title:titleRect,content:contentRect,nav:rect(nav),navVisible:visible(nav),navButtonRects:buttons.map(rect),more:rect(more),moreVisible:visible(more),account:rect(account),accountVisible:visible(account),slot:slotRect,slotVisible:visible(slot),fab:fabRect,fabVisible:visible(fab),label:labelRect,labelVisible:visible(label),fabInsideSlot:contains(slotRect,fabRect),labelInsideSlot:contains(slotRect,labelRect),slotInsideTop:contains(topRect,slotRect),fabTitleOverlap:intersects(fabRect,titleRect),labelTitleOverlap:intersects(labelRect,titleRect),fabContentOverlap:intersects(fabRect,contentRect),labelContentOverlap:intersects(labelRect,contentRect)}})()`,returnByValue:true});
  ...
  if(!state.slotVisible||!state.slot||state.slot.width<54||state.slot.height<60)throw new Error(`Structural assistant slot missing: ${JSON.stringify(state)}`);
  ...
  if(!state.slotInsideTop||!state.fabInsideSlot||!state.labelInsideSlot)throw new Error(`Assistant is not contained by its structural header slot: ${JSON.stringify(state)}`);
  if(state.fabTitleOverlap||state.labelTitleOverlap)throw new Error(`Unified AI Mini overlaps top title: ${JSON.stringify(state)}`);
  if(state.fabContentOverlap||state.labelContentOverlap)throw new Error(`Unified AI Mini overlaps module content: ${JSON.stringify(state)}`);
