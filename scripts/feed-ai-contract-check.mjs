...
const newsCss=read('src/news-rotator.css');
const checks=[
  ['news rotator loads a rolling pool',news.includes("p_limit:20")],
  ['news rotator auto-advances',news.includes('ROTATE_MS=8000')],
  ['desktop news displays two cards per viewport',newsCss.includes('flex:0 0 calc((100% - var(--news-gap))/2)')],
  ...
  ['TCM news retention remains independent',retention.includes('private.tcm_news_retention_v2()')],
...
if(failed.length){console.error(`Feed/ACC/Garden contract failed: ${failed.map(([name])=>name).join(', ')}`);process.exit(1)}
console.log('Feed compact paging + ACC AI Operations + garden watering contract passed.');
