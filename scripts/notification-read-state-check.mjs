...
const checks=[
  ...
  ['desktop and mobile secondary notification navigation use canonical unread state',app.match(/notification-nav-count/g)?.length>=1&&app.match(/aria-label=\{notificationAria\}/g)?.length>=1&&app.includes('onClick={requestNotifications}')&&app.includes("notificationBadge?`${notificationBadge} chưa đọc`:'Cập nhật của bạn'")],
